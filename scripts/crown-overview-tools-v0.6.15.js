...
function removePanel() {
  ...
  function buildCharacterForm(details = {}, selectedTileId = "") {
    ...
    return `<form>
      ...
      <div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:8px;">
        <div class="form-group"><label>Land Move</label><input type="number" name="landMovement" value="${escapeHtml(details.landMovement ?? details.movementMax ?? 2)}" step="1" style="width:100%;" /></div>
        <div class="form-group"><label>Naval Move</label><input type="number" name="navalMovement" value="${escapeHtml(details.navalMovement ?? 2)}" step="1" style="width:100%;" /></div>
  ...
  async function saveCharacterFlags(token, details, entry, ownerUser = null) {
    ...
    const movementMax = numberOrBlank(characterData.landMovement) === "" ? Number(currentPiece.movementMax || 2) : Number(characterData.landMovement);
    const updatedPiece = {
      ...currentPiece,
  ...
  async function spawnCharacterToken(details, entry, ownerUser = null) {
    ...
    const movementMax = numberOrBlank(characterData.landMovement) === "" ? 2 : Number(characterData.landMovement);
    const pieceData = {
  ...
  async function createCharacter() {
    ...
    const blank = normalizeCharacterDetails({ landMovement: 2, navalMovement: 2, status: "Alive", visibility: "Owner+GM" });
    ...
    const result = await new Promise(resolve => {
      new Dialog({
        ...
        content: `<form>
          ...
          <div class="form-group"><label>Movement Points Per Turn</label><input type="number" name="movementMax" value="${escapeHtml(piece.movementMax ?? 2)}" min="0" step="1" style="width:100%;" /></div>
