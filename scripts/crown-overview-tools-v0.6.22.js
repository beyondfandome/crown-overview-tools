(() => {
  ...
  const SIEGE_ENGINE_TYPES = [
    { key: "Battering Ram", label: "Battering Ram", requiredTier: 1, capacity: 1, baseBonus: 2, note: "+2% storm chance; +2% more against any fortified target" },
    { key: "Ballista", label: "Ballista", requiredTier: 2, capacity: 1, baseBonus: 1, casualtyReduction: 1, note: "+1% storm chance; reduces attacker casualties by 1 percentage point each (max -5%)" },
    { key: "Onager", label: "Onager", requiredTier: 2, capacity: 2, baseBonus: 6, developedBonus: 2, note: "+6% storm chance; +2% more against Holdfasts, Towns, and Cities" },
    { key: "Trebuchet", label: "Trebuchet", requiredTier: 3, capacity: 3, baseBonus: 10, majorFortBonus: 5, note: "+10% storm chance; +5% more against Town/City targets or Fortification level 2-3" }
  ];
  const MAX_SIEGE_ENGINE_BONUS = 30;
  const MAX_BALLISTA_CASUALTY_REDUCTION = 5;
  ...
  const SIEGE_DEVELOPMENT_BASE_DC = {
    ruins: 15,
    village: 25,
    holdfast: 35,
    town: 45,
  ...
  const BUILDING_LINES = [
    { key: "fields", group: "Economy", label: "Resow The Fields", levels: [
      ...
      { level: 3, name: "Granary", income: { Food: 15 }, effect: "+15 Food" }
    ]},
    { key: "orchard", group: "Economy", label: "Replant The Vineyard", levels: [
      ...
      { level: 3, name: "Winery", income: { Gold: 3, Food: 9 }, effect: "+3 Gold, +9 Food" }
    ]},
    ...
    { key: "pastures", group: "Economy", label: "Pen The Lost Herds", levels: [
      ...
      { level: 3, name: "Slaughterhouses", income: { Food: 15 }, effect: "+15 Food" }
    ]},
    ...
    { key: "barracks", group: "Military", label: "Barracks", levels: [
      { level: 1, name: "Town Guards", trainingCapacity: { "Light Infantry": 500 }, effect: "Training capacity: 500 Light Infantry within the normal manpower cap" },
      { level: 2, name: "Town Barracks", trainingCapacity: { "Light Infantry": 1000, Spearmen: 1000 }, effect: "Training capacity: 1,000 Light Infantry; 1,000 Spearmen within the normal manpower cap" },
      { level: 3, name: "Infantry Schools", trainingCapacity: { "Light Infantry": 1000, Spearmen: 1000, "Heavy Infantry": 250, Pikemen: 250 }, effect: "Training capacity: 1,000 Light Infantry; 1,000 Spearmen; 250 Heavy Infantry; 250 Pikemen" }
    ]},
    { key: "archery", group: "Military", label: "Archery Range", levels: [
      { level: 1, name: "Hunters Lodge", trainingCapacity: { Archers: 500 }, effect: "Training capacity: 500 Archers within the normal manpower cap" },
      { level: 2, name: "Archery Range", trainingCapacity: { Archers: 1000, Crossbowmen: 1000 }, effect: "Training capacity: 1,000 Archers; 1,000 Crossbowmen within the normal manpower cap" },
      { level: 3, name: "Master Fletchers", trainingCapacity: { Archers: 1500, Crossbowmen: 1500 }, effect: "Training capacity: 1,500 Archers; 1,500 Crossbowmen within the normal manpower cap" }
    ]},
    { key: "stable", group: "Military", label: "Stable", levels: [
      { level: 1, name: "Scout Stable", aliases: ["Scouts Stable"], trainingCapacity: { "Light Cavalry": 250 }, effect: "Training capacity: 250 Light Cavalry within the normal manpower cap" },
      { level: 2, name: "Lancers Training Grounds", trainingCapacity: { "Light Cavalry": 500, Lancers: 500 }, effect: "Training capacity: 500 Light Cavalry; 500 Lancers within the normal manpower cap" },
      { level: 3, name: "Knight's Holdfast", aliases: ["Knights Holdfasts"], trainingCapacity: { "Light Cavalry": 500, Lancers: 500, "Heavy Cavalry": 200 }, effect: "Training capacity: 500 Light Cavalry; 500 Lancers; 200 Heavy Cavalry within the normal manpower cap" }
    ]},
    ...
    { key: "watchtower", group: "Military", label: "Watchtowers", levels: [
      { level: 1, name: "Watchtowers", income: {}, movementCostModifier: 0.5, siegeModifier: 5, effect: "+5 siege DC; +0.5 enemy movement cost" },
      { level: 2, name: "Holdfasts", income: {}, movementCostModifier: 1, siegeModifier: 10, effect: "+10 siege DC; +1 enemy movement cost" },
      { level: 3, name: "Castles", income: {}, movementCostModifier: 1.5, siegeModifier: 15, effect: "+15 siege DC; +1.5 enemy movement cost" }
    ]},
    { key: "siege", group: "Military", label: "Siege Workshop", levels: [
      { level: 1, name: "Ram Specialists", income: {}, siegeCapacity: 5, effect: "+5 House Siege Capacity; unlock Battering Rams" },
      { level: 2, name: "Carpenters Guild", income: {}, siegeCapacity: 10, effect: "+10 House Siege Capacity; unlock Ballistae and Onagers" },
      { level: 3, name: "Siege Workshop", income: {}, siegeCapacity: 15, effect: "+15 House Siege Capacity; unlock Trebuchets" }
    ]},
  ...
  const BUILDING_TREE_DEFINITIONS = {
    ...
    military: { maxLevel: 3, lines: ["Barracks", "Archery Range", "Stable", "Siege Workshop"], notes: "Barracks, Archery Range, and Stable provide trained-troop capacity; Siege Workshop provides House Siege Capacity and engine unlocks." },
    ...
    fortification: { maxLevel: 3, movementCostIncreasePerLevel: 0.5, quickSiegeDcIncreasePerLevel: 5 }
  };
  ...
  function getBuildingDataByName(house = {}) {
    const map = new Map();
    for (const item of Array.isArray(house.buildingData) ? house.buildingData : []) {
      const name = String(item?.name || item?.building || "");
      if (name) map.set(name, item);
    }
    return map;
  }

  function isBuildingMetaActive(meta = null, clock = getClock()) {
    if (!meta) return true;
    const activeSort = roundSortValueFromBuildMeta(meta);
    if (!activeSort) return true;
    return activeSort <= roundSortValue(clock || getDefaultClock());
  }

  function getActiveBuiltBuildings(house = {}, clock = getClock()) {
    const built = Array.isArray(house.builtBuildings) ? house.builtBuildings.map(String) : [];
    const detailsByName = getBuildingDataByName(house);
    return built.filter(name => isBuildingMetaActive(detailsByName.get(String(name)), clock));
  }

  function getPendingBuiltBuildings(house = {}, clock = getClock()) {
    const built = Array.isArray(house.builtBuildings) ? house.builtBuildings.map(String) : [];
    const detailsByName = getBuildingDataByName(house);
    return built.filter(name => !isBuildingMetaActive(detailsByName.get(String(name)), clock));
  }

  function getActiveBuildingLineState(house = {}, clock = getClock()) {
    const activeBuilt = getActiveBuiltBuildings(house, clock);
    const activeNames = new Set(activeBuilt.map(name => normalize(name)));
    const activeSlots = Array.isArray(house.buildingSlots)
      ? house.buildingSlots.filter(item => activeNames.has(normalize(item?.name || item?.building || "")))
      : [];
    return getBuildingLineState({ ...house, builtBuildings: activeBuilt, buildingSlots: activeSlots });
  }

  function getActiveBuildingSlotCount(house = {}, clock = getClock()) {
    const activeBuilt = getActiveBuiltBuildings(house, clock);
    const activeNames = new Set(activeBuilt.map(name => normalize(name)));
    const activeSlots = Array.isArray(house.buildingSlots)
      ? house.buildingSlots.filter(item => activeNames.has(normalize(item?.name || item?.building || "")))
      : [];
    return getBuildingSlotCount({ ...house, builtBuildings: activeBuilt, buildingSlots: activeSlots });
  }
  ...
  function describeBuildingOption(line, level, mode, house = {}) {
    const action = mode.startsWith("Upgrade") ? "Upgrade to" : "Build";
    return `${action} ${level.name} — Tier ${level.level}`;
  }
  ...
  function makeBuildingOption(line, level, mode, house = {}, piece = {}) {
    ...
    const isUpgrade = mode.startsWith("Upgrade");
    const label = `${isUpgrade ? "Upgrade to" : "Build"} ${level.name} — Tier ${level.level}${lockedByStatecraft ? " — LOCKED" : ""}`;
    return {
      group: line.group,
      category: line.group,
      value: level.name,
      label,
      disabled: lockedByStatecraft,
      lockedReason: lockedByStatecraft ? `${piece.name || "This character"} has Statecraft ${pieceStatecraft}; needs ${statecraftRequired}.` : "",
      mode: isUpgrade ? "upgrade" : "build",
      lineKey: line.key,
      lineLabel: line.label,
      tier: Number(level.level || 0),
      statecraftRequired,
      cost: getBuildingTierCost(level.level, line, house),
      effect: getBuildingDisplayEffect(line, level, house),
      currentName: current?.level?.name || "",
      currentTier: Number(current?.level?.level || 0),
      currentEffect: current ? getBuildingDisplayEffect(line, current.level, house) : ""
    };
  }

  function getMineFoodBuildingConflict(house = {}, targetBuilding = "") {
    // Resource-land exclusivity: these four building lines are mutually exclusive
    // from Tier 1 onward. Once one line exists on a tile, none of the other three
    // can be started there. Upgrades within the existing line remain legal.
    const state = getBuildingLineState(house);
    const targetMeta = getBuildingMetaByName(targetBuilding);
    const targetLineKey = targetMeta?.line?.key || "";
    const exclusiveLines = new Set(["fields", "mine", "pastures", "fishery"]);

    if (!exclusiveLines.has(targetLineKey)) return "";

    for (const lineKey of exclusiveLines) {
      if (lineKey === targetLineKey) continue;
      const current = state.get(lineKey);
      if (current) {
        return `${targetBuilding} cannot be built on this tile because it already has ${current.level.name}. Fields/Granaries, Mines, Pastures/Slaughterhouses, and Fishing Wharfs/Deep Sea Fishing Boats are mutually exclusive from Tier 1 onward.`;
      }
    }
    return "";
  }
  ...
  function buildCatalogBuildingOptions(house = {}, piece = {}) {
    ...
    for (const legacy of BUILDINGS) {
      if (!built.includes(legacy) && !getBuildingMetaByName(legacy) && slotCount < 4) {
        options.push({ group: "Legacy", category: "Legacy", value: legacy, label: `Build ${legacy}`, disabled: false, mode: "build", lineKey: "legacy", lineLabel: "Legacy", tier: 1, statecraftRequired: 0, cost: getBuildingCost(legacy, house), effect: getBuildingRule(legacy, house)?.effect || "Legacy building", currentName: "", currentTier: 0, currentEffect: "" });
      }
    }

    for (const option of options) {
      const conflict = getMineFoodBuildingConflict(house, option.value);
      if (conflict) {
        option.disabled = true;
        option.lockedReason = conflict;
        if (!String(option.label || "").includes("LOCKED")) option.label = `${option.label} — LOCKED`;
      }
    }
  ...
  function renderOptionGroups(options) {
    ...
    for (const [group, rows] of groups.entries()) {
      ...
      for (const option of rows) html += `<option value="${escapeHtml(option.value)}" ${option.disabled ? "disabled" : ""}>${escapeHtml(option.label)}</option>`;
      html += `</optgroup>`;
    }
    return html;
  }
  ...
  function strategicMovementLockReason(piece = {}) {
    ...
    if (piece.lastRaidRoundKey && String(piece.lastRaidRoundKey) === String(roundKey)) {
...
function renderPanel() {
  ...
  const gmButtons = game.user.isGM ? `
    <div class="coa-panel-section coa-panel-gm-section">
      <div class="coa-panel-section-title">GM: Round Management</div>
      <button data-coa-action="roundClock"><strong>Round Clock</strong></button>
    ...
    <div class="coa-panel-section coa-panel-gm-section">
      <div class="coa-panel-section-title">GM: Map Tools & Maintenance</div>
  ...
  panel.innerHTML = `
    ...
    <div class="coa-panel-section coa-panel-player-section">
      <div class="coa-panel-section-title">Player: Characters & Movement</div>
...
function removePanel() {
  ...
  function getUserByIdOrName(userId = "", userName = "") {
    const id = String(userId || "").trim();
    const name = normalize(userName || "");
    if (id && game.users.get(id)) return game.users.get(id);
    if (name) return game.users.contents.find(user => normalize(user.name) === name) || null;
    return null;
  }
  ...
  function ensureEconomyBucket(bucketMap, user) {
    ...
    if (!bucketMap.has(key)) {
      bucketMap.set(key, { user, applied: 0, totals: {}, lines: [], upkeepTotals: {}, upkeepLines: [] });
    }
    return bucketMap.get(key);
  }

  function buildEconomyCollectionContent({ title = "Economy Collected", dateLabel = "", seasonMultiplier = "", applied = 0, totals = {}, lines = [], upkeepTotals = {}, upkeepLines = [] } = {}) {
    return `<h2>${escapeHtml(title)}</h2><p><strong>Date:</strong> ${escapeHtml(dateLabel)}</p><p><strong>Season Multiplier:</strong> ${escapeHtml(seasonMultiplier)}</p><p><strong>Tiles Paid:</strong> ${escapeHtml(applied)}</p><p><strong>Total Income:</strong> ${escapeHtml(resourceMapToText(totals))}</p><ul>${lines.join("") || "<li>No economy-enabled tiles produced resources.</li>"}</ul><h3>Military Upkeep</h3><p><strong>Total Upkeep:</strong> ${escapeHtml(resourceMapToText(upkeepTotals || {}, "None"))}</p><ul>${upkeepLines.join("") || "<li>No active armies or navies charged upkeep for this scope.</li>"}</ul>`;
  }
  ...
  function getProvinceManpowerMax(house = {}, clock = getClock()) {
    let bonus = 0;
    const state = getActiveBuildingLineState(house, clock).get("mustering");
  ...
  function getHouseManpowerSummaryForCharacter(characterToken) {
    ...
    for (const entry of entries) {
      const house = getHouseData(entry.drawing) || {};
      rawCurrent += getProvinceManpowerCurrent(house);
      rawMax += getProvinceManpowerMax(house);
    }
    const influence = getInfluenceSummaryForEntries(entries);
    const max = Math.floor(rawMax * influence.levyMultiplier);
    const current = Math.min(rawCurrent, max);
    return { entries, current, max, rawCurrent, rawMax, influence };
  }
  ...
  function getHouseDrydockLevelForCharacter(characterToken) {
    ...
    for (const entry of entries) {
      const house = getHouseData(entry.drawing) || {};
      const state = getActiveBuildingLineState(house).get("drydock");
  ...
  function getSiegeCapacityForEntries(entries = []) {
    let total = 0;
    for (const entry of entries) {
      const house = getHouseData(entry.drawing) || {};
      const state = getActiveBuildingLineState(house).get("siege");
      total += Math.max(0, Number(state?.level?.siegeCapacity || 0));
    }
    return Math.max(0, Math.floor(total));
  }

  function getSiegeInfrastructureLevelForEntries(entries = []) {
    let level = 0;
    for (const entry of entries) {
      const house = getHouseData(entry.drawing) || {};
      const state = getActiveBuildingLineState(house).get("siege");
  ...
  function getSiegeInfrastructureSummary(level = 0) {
    const n = Math.max(0, Math.min(3, Number(level || 0)));
    if (n <= 0) return "No Siege Workshop: no siege equipment unlocked";
    if (n === 1) return "Ram Specialists: Battering Rams";
    if (n === 2) return "Carpenters Guild: adds Ballistae and Onagers";
    return "Siege Workshop: adds Trebuchets";
  }

  function getSiegeEquipment(piece = {}) {
    const raw = piece?.siegeEquipment || piece?.siegeComposition || {};
    const result = {};
    let hasStored = false;
    for (const engine of SIEGE_ENGINE_TYPES) {
      const value = Math.max(0, Math.floor(Number(raw?.[engine.key] ?? raw?.[engine.label] ?? 0)));
      result[engine.key] = value;
      if (value > 0) hasStored = true;
    }
    // Compatibility with armies created before v0.6.4: old numeric engines behave as Rams.
    if (!hasStored) {
      const legacy = Math.max(0, Math.floor(Number(piece?.siegeEngines || 0)));
      if (legacy > 0) result["Battering Ram"] = legacy;
    }
    return result;
  }

  function getSiegeEquipmentCount(equipment = {}) {
    return SIEGE_ENGINE_TYPES.reduce((sum, engine) => sum + Math.max(0, Math.floor(Number(equipment?.[engine.key] || 0))), 0);
  }

  function getSiegeEquipmentCapacityUsed(equipment = {}) {
    return SIEGE_ENGINE_TYPES.reduce((sum, engine) => sum + Math.max(0, Math.floor(Number(equipment?.[engine.key] || 0))) * Number(engine.capacity || 0), 0);
  }

  function siegeEquipmentText(equipment = {}, emptyText = "None") {
    const parts = [];
    for (const engine of SIEGE_ENGINE_TYPES) {
      const count = Math.max(0, Math.floor(Number(equipment?.[engine.key] || 0)));
      if (count > 0) parts.push(`${engine.label}: ${count.toLocaleString()}`);
    }
    return parts.length ? parts.join("; ") : emptyText;
  }

  function calculateSiegeEquipmentUpkeep(equipment = {}) {
    const capacity = getSiegeEquipmentCapacityUsed(equipment);
    return capacity > 0 ? { Gold: Math.ceil(capacity / 5) } : {};
  }

  function getCommittedSiegeCapacityForHouse(houseKey, excludeMusterId = "", excludeArmyTokenId = "") {
    let used = 0;
    for (const token of getArmyTokens()) {
      if (excludeArmyTokenId && String(token.document.id) === String(excludeArmyTokenId)) continue;
      const piece = getWorldPiece(token) || {};
      if (normalize(piece.house || piece.faction || "") !== normalize(houseKey)) continue;
      used += getSiegeEquipmentCapacityUsed(getSiegeEquipment(piece));
    }
    for (const token of getCharacterTokens()) {
      const pending = getPendingArmyMuster(token);
      if (!pending || pending.status !== "pending" || String(pending.id || "") === String(excludeMusterId || "")) continue;
      if (normalize(pending.house || "") !== normalize(houseKey)) continue;
      used += getSiegeEquipmentCapacityUsed(getSiegeEquipment(pending));
    }
    return Math.max(0, Math.floor(used));
  }

  function getAvailableSiegeCapacity(characterToken, excludeMusterId = "", excludeArmyTokenId = "") {
    const entries = getManpowerEntriesForCharacter(characterToken);
    const total = getSiegeCapacityForEntries(entries);
    const tier = getSiegeInfrastructureLevelForEntries(entries);
    const used = getCommittedSiegeCapacityForHouse(getCharacterHouseKey(characterToken), excludeMusterId, excludeArmyTokenId);
    return { entries, total, used, available: Math.max(0, total - used), tier };
  }

  function buildSiegeEquipmentInputs(summary = {}, currentEquipment = {}) {
    const tier = Math.max(0, Number(summary.tier || 0));
    const available = Math.max(0, Number(summary.available || 0));
    return SIEGE_ENGINE_TYPES.map(engine => {
      const current = Math.max(0, Math.floor(Number(currentEquipment?.[engine.key] || 0)));
      const unlockedByTier = tier >= Number(engine.requiredTier || 0);
      const maxQty = Math.max(current, Math.floor(available / Math.max(1, Number(engine.capacity || 1))));
      const unlocked = unlockedByTier && (available >= Number(engine.capacity || 1) || current > 0);
      const lockText = !unlockedByTier
        ? `LOCKED — requires Siege Workshop tier ${engine.requiredTier}`
        : (!unlocked ? "LOCKED — no available House Siege Capacity" : `Uses ${engine.capacity} Siege Capacity each`);
      return `<div class="form-group" style="${unlocked ? "" : "opacity:0.55;"}"><label>${escapeHtml(engine.label)}${unlocked ? "" : " — LOCKED"}</label><input type="number" name="siege_${escapeHtml(engine.key)}" value="${escapeHtml(current)}" min="0" max="${escapeHtml(maxQty)}" step="1" style="width:100%;" ${unlocked ? "" : "disabled"} /><p class="notes">${escapeHtml(lockText)}. ${escapeHtml(engine.note)}</p></div>`;
    }).join("");
  }

  function readSiegeEquipmentForm(form) {
    const equipment = {};
    for (const engine of SIEGE_ENGINE_TYPES) {
      const input = form.elements[`siege_${engine.key}`];
      equipment[engine.key] = Math.max(0, Math.floor(Number(input?.value || 0)));
    }
    return equipment;
  }

  function validateSiegeEquipmentSelection(equipment = {}, summary = {}) {
    const tier = Math.max(0, Number(summary.tier || 0));
    for (const engine of SIEGE_ENGINE_TYPES) {
      const count = Math.max(0, Math.floor(Number(equipment?.[engine.key] || 0)));
      if (count > 0 && tier < Number(engine.requiredTier || 0)) throw new Error(`${engine.label} requires Siege Workshop tier ${engine.requiredTier}. Your House infrastructure is tier ${tier}.`);
    }
    const capacity = getSiegeEquipmentCapacityUsed(equipment);
    const available = Math.max(0, Number(summary.available || 0));
    if (capacity > available) throw new Error(`Siege equipment exceeds House Siege Capacity: ${capacity} selected, ${available} available.`);
    return { capacity, upkeep: calculateSiegeEquipmentUpkeep(equipment) };
  }
  ...
  function calculateArmyUpkeep(composition = {}, siegeEquipment = {}) {
    ...
    return normalizeResourceMap(addResourceMaps(totals, calculateSiegeEquipmentUpkeep(siegeEquipment)));
  }
  ...
  function sanitizeEmbarkCompositionChoice(choice = {}, maxComposition = {}, capacity = 0) {
    const limit = Math.max(0, Math.floor(Number(capacity || 0)));
    const result = {};
    let total = 0;
    for (const troop of ARMY_TROOP_TYPES) {
      const max = Math.max(0, Math.floor(Number(maxComposition[troop.key] || 0)));
      const raw = Math.max(0, Math.floor(Number(choice[troop.key] ?? choice[troop.label] ?? 0)));
      const value = Math.min(max, raw);
      result[troop.key] = value;
      total += value;
    }
    if (total <= 0) throw new Error("Choose at least some troops to embark.");
    if (total > limit) throw new Error(`Selected troops exceed fleet capacity: ${total.toLocaleString()} selected, ${limit.toLocaleString()} capacity available.`);
    return { composition: result, total };
  }

  function defaultEmbarkCompositionForCapacity(composition = {}, capacity = 0) {
    const result = {};
    for (const troop of ARMY_TROOP_TYPES) result[troop.key] = 0;
    let remaining = Math.max(0, Math.floor(Number(capacity || 0)));
    for (const troop of ARMY_TROOP_TYPES) {
      if (remaining <= 0) break;
      const available = Math.max(0, Math.floor(Number(composition[troop.key] || 0)));
      const keep = Math.min(available, remaining);
      result[troop.key] = keep;
      remaining -= keep;
    }
    return result;
  }

  async function chooseEmbarkCompositionForCapacity(armyPiece = {}, fleetPiece = {}, composition = {}, capacity = 0) {
    const maxCapacity = Math.max(0, Math.floor(Number(capacity || 0)));
    const defaults = defaultEmbarkCompositionForCapacity(composition, maxCapacity);
    const originalTotal = getArmyTotalStrength(composition);
    const rows = ARMY_TROOP_TYPES.map(troop => {
      const available = Math.max(0, Math.floor(Number(composition[troop.key] || 0)));
      const disabled = available <= 0 ? "disabled" : "";
      return `<tr>
        <td style="padding:4px 6px;border-bottom:1px solid #777;">${escapeHtml(troop.label)}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #777;text-align:right;">${escapeHtml(available.toLocaleString())}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #777;"><input type="number" name="troop_${escapeHtml(troop.key)}" value="${escapeHtml(defaults[troop.key] || 0)}" min="0" max="${escapeHtml(available)}" step="50" style="width:100%;" ${disabled}></td>
      </tr>`;
    }).join("");

    return await new Promise(resolve => {
      new Dialog({
        title: "Choose Embarking Troops",
        content: `<form>
          <p><strong>${escapeHtml(fleetPiece.name || "Selected Navy")}</strong> can carry only <strong>${escapeHtml(maxCapacity.toLocaleString())}</strong> more troops.</p>
          <p><strong>${escapeHtml(armyPiece.name || "Selected Army")}</strong> has <strong>${escapeHtml(originalTotal.toLocaleString())}</strong> troops. Choose which troops embark. Any troops not selected are dispersed/dismissed and their upkeep is removed.</p>
          <table style="width:100%;border-collapse:collapse;margin-top:8px;">
            <thead><tr><th style="text-align:left;">Troop Type</th><th style="text-align:right;">Available</th><th style="text-align:left;">Embark</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
          <p class="notes">Selected total must be ${escapeHtml(maxCapacity.toLocaleString())} troops or fewer. You can choose Mob, trained troops, or a mix.</p>
        </form>`,
        buttons: {
          embark: { label: "Embark Selected", callback: html => {
            try {
              const form = html[0].querySelector("form");
              const choice = {};
              for (const troop of ARMY_TROOP_TYPES) choice[troop.key] = Number(form.elements[`troop_${troop.key}`]?.value || 0);
              resolve(sanitizeEmbarkCompositionChoice(choice, composition, maxCapacity));
            } catch (err) {
        ...
        default: "embark"
      }, { width: 680, height: "auto", resizable: true }).render(true);
    });
  }
  ...
  async function createArmyMusterRequestForCharacter(characterToken) {
    ...
    const piece = getWorldPiece(characterToken);
    if (!getClock()) {
      ui.notifications.warn("Initialize the World Round Clock before beginning an army muster.");
      return null;
    }
    const martial = getCharacterMartialValue(characterToken);
    ...
    const siege = getAvailableSiegeCapacity(characterToken);
    ...
    const result = await new Promise(resolve => {
      new Dialog({
        ...
        content: `<form>
          ...
          <hr><h3 style="margin:6px 0;">Siege Equipment</h3>
          <p class="notes" style="margin-top:0;">Siege equipment uses House-wide Siege Capacity. Fielded equipment adds 1 Gold upkeep per 5 capacity (rounded up).</p>
          <div style="display:grid;grid-template-columns:repeat(2, 1fr);gap:8px;">${buildSiegeEquipmentInputs(siege)}</div>
        ...
        default: "save"
      }, { width: 820, height: 900, resizable: true }).render(true);
    });
    ...
    let siegeValidation;
    try { siegeValidation = validateSiegeEquipmentSelection(result.siegeEquipment, siege); }
    catch (err) { ui.notifications.error(err.message || "Invalid siege equipment selection."); return null; }
    const upkeep = calculateArmyUpkeep(result.composition, result.siegeEquipment);
    return {
      ...
      upkeep,
      siegeEquipment: result.siegeEquipment,
      siegeCapacityUsed: siegeValidation.capacity,
      siegeUpkeep: siegeValidation.upkeep,
      siegeEngines: getSiegeEquipmentCount(result.siegeEquipment),
  ...
  function validateArmyMusterCapacity(characterToken, muster = {}) {
    const martial = getCharacterMartialValue(characterToken);
    const commandCap = Math.max(0, Math.floor(martial * 250));
    const manpower = getHouseManpowerSummaryForCharacter(characterToken);
    const totalStrength = Math.max(0, Number(muster.totalStrength || getArmyTotalStrength(muster.composition || {})));
    if (totalStrength > commandCap) throw new Error(`Commander capacity exceeded: ${totalStrength.toLocaleString()} men requested, but Martial ${martial} can command ${commandCap.toLocaleString()} men.`);
    if (totalStrength > manpower.current) throw new Error(`Not enough manpower: ${totalStrength.toLocaleString()} requested, ${manpower.current.toLocaleString()} available.`);
    const training = getAvailableTrainingCapacity(characterToken, muster.id || "");
    for (const troop of ARMY_TROOP_TYPES) {
      if (troop.key === "Mob") continue;
      const selected = Math.max(0, Number(muster.composition?.[troop.key] || 0));
      if (selected > Number(training.available[troop.key] || 0)) throw new Error(`${troop.label} exceeds available training capacity.`);
    }
    const siege = getAvailableSiegeCapacity(characterToken, muster.id || "");
    const siegeValidation = validateSiegeEquipmentSelection(getSiegeEquipment(muster), siege);
    return { commandCap, manpower, training, siege, siegeValidation, totalStrength };
  }
  ...
  async function summonArmy() {
    ...
    if (!game.user.isGM) {
      ...
      if (!gm) {
        ui.notifications.warn("An active GM connection is required to reserve realm manpower and create the future army token.");
        return;
      }
      game.socket.emit(SOCKET_NAME, {
        type: "armyMusterRequest",
        targetGmId: gm.id,
      ...
      ui.notifications.info(`Beginning ${muster.armyName}. Ready ${muster.readyDateLabel || "next round"}.`);
      return;
    }
    try {
      validateArmyMusterCapacity(token, muster);
      await reserveManpowerForMuster(token, muster);
    } catch (err) {
      ui.notifications.error(err.message || "Could not reserve manpower.");
      return;
    }
    await savePendingArmyMuster(token, muster);
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ alias: "Crown Armies" }),
      whisper: [game.user.id],
      content: `<h2>Army Muster Begun</h2><p><strong>Commander:</strong> ${escapeHtml(muster.linkedCharacterName)}</p><p><strong>Army:</strong> ${escapeHtml(muster.armyName)}</p><p><strong>Strength:</strong> ${escapeHtml(muster.totalStrength.toLocaleString())}</p><p><strong>Composition:</strong> ${escapeHtml(armyCompositionText(muster.composition))}</p><p><strong>Siege Equipment:</strong> ${escapeHtml(siegeEquipmentText(getSiegeEquipment(muster)))}</p><p><strong>Upkeep when active:</strong> ${escapeHtml(resourceMapToText(muster.upkeep))}</p><p><strong>Ready:</strong> ${escapeHtml(muster.readyDateLabel || "next round")}</p><p>Manpower and specialist capacity are reserved immediately. No GM approval is required.</p>`
    });
  }
  ...
  async function createNavyMusterRequestForCharacter(characterToken) {
    ...
    return {
      ...
      movementMax: Math.max(1, Number(navalMovement || 2)),
      movementUsed: Math.max(1, Number(navalMovement || 2)),
  ...
  async function handleArmyMusterRequest(message) {
    ...
    if (!token) {
      const reason = `Army muster failed: ${message.tokenName || message.tokenId} not found.`;
      if (requester && game.socket) game.socket.emit(SOCKET_NAME, { type: "playerNotification", targetUserId: requester.id, level: "error", message: reason });
      return;
    }

    try {
      validateArmyMusterCapacity(token, message.muster);
      await reserveManpowerForMuster(token, message.muster);
      await savePendingArmyMuster(token, message.muster);
    } catch (err) {
      console.error("Army muster could not begin", err, message);
      const reason = err.message || "Could not reserve manpower for army muster.";
      if (requester && game.socket) game.socket.emit(SOCKET_NAME, { type: "playerNotification", targetUserId: requester.id, level: "error", message: `Army muster failed: ${reason}` });
      await ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ alias: "Crown Armies" }),
        whisper: requester ? [requester.id, ...ChatMessage.getWhisperRecipients("GM").map(user => user.id)] : ChatMessage.getWhisperRecipients("GM").map(user => user.id),
        content: `<h2>Army Muster Failed</h2><p><strong>Commander:</strong> ${escapeHtml(message.muster?.linkedCharacterName || token.document.name)}</p><p><strong>Reason:</strong> ${escapeHtml(reason)}</p>`
      });
      return;
    }

    if (requester && game.socket) {
      game.socket.emit(SOCKET_NAME, {
        type: "playerNotification",
        targetUserId: requester.id,
        level: "info",
        message: `${message.muster?.armyName || "Army"} muster begun. Ready ${message.muster?.readyDateLabel || "next round"}.`
      });
    }
    ...
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ alias: "Crown Armies" }),
      whisper: requester ? [requester.id, ...ChatMessage.getWhisperRecipients("GM").map(user => user.id)] : ChatMessage.getWhisperRecipients("GM").map(user => user.id),
      content: `<h2>Army Muster Begun</h2><p><strong>Player:</strong> ${escapeHtml(message.requesterUserName || "Unknown")}</p><p><strong>Commander:</strong> ${escapeHtml(message.muster?.linkedCharacterName || token.document.name)}</p><p><strong>Army:</strong> ${escapeHtml(message.muster?.armyName || "Host")}</p><p><strong>Strength:</strong> ${escapeHtml(Number(message.muster?.totalStrength || 0).toLocaleString())}</p><p><strong>Composition:</strong> ${escapeHtml(armyCompositionText(message.muster?.composition || {}))}</p><p><strong>Siege Equipment:</strong> ${escapeHtml(siegeEquipmentText(getSiegeEquipment(message.muster || {})))}</p><p><strong>Ready:</strong> ${escapeHtml(message.muster?.readyDateLabel || "next round")}</p><p>This muster will be processed automatically by the Round Clock. No GM approval is required.</p>`
    });
  }
  ...
  async function spawnArmyTokenFromMuster(characterToken, muster) {
    ...
    const armyPiece = {
      ...
      upkeep: muster.upkeep || calculateArmyUpkeep(muster.composition || {}, getSiegeEquipment(muster)),
      siegeEquipment: getSiegeEquipment(muster),
      siegeCapacityUsed: getSiegeEquipmentCapacityUsed(getSiegeEquipment(muster)),
      siegeUpkeep: calculateSiegeEquipmentUpkeep(getSiegeEquipment(muster)),
      siegeEngines: getSiegeEquipmentCount(getSiegeEquipment(muster)),
  ...
  async function spawnNavyTokenFromMuster(characterToken, muster) {
    ...
    const fleetPiece = {
      ...
      followCharacter: muster.followCharacter !== false,
      detached: false,
      movementMax: Number(muster.movementMax || getCharacterNavalMovementValue(characterToken) || 2),
      movementUsed: 0,
  ...
  function roundSortValueFromRoundKey(roundKey = "") {
    const parts = String(roundKey || "").split("|");
    if (parts.length < 3) return null;
    const year = Number(parts[0]);
    const season = String(parts[1] || "").trim();
    const round = Number(parts[2]);
    if (!Number.isFinite(year) || !Number.isFinite(round)) return null;
    const index = ROUND_ORDER.findIndex(item => normalize(item.season) === normalize(season) && Number(item.round) === round);
    if (index < 0) return null;
    return roundSortValueFromParts(year, index);
  }

  function getMusterReadyRoundSort(muster = {}) {
    const explicit = Number(muster.readyRoundSort);
    if (Number.isFinite(explicit)) return explicit;
    const fromReadyKey = roundSortValueFromRoundKey(muster.readyRoundKey);
    if (fromReadyKey !== null) return fromReadyKey;

    const requested = Number(muster.requestedRoundSort);
    if (Number.isFinite(requested)) return requested + 1;
    const fromRequestedKey = roundSortValueFromRoundKey(muster.requestedRoundKey);
    if (fromRequestedKey !== null) return fromRequestedKey + 1;
    return null;
  }

  function isArmyMusterReady(muster = {}, clock = getClock()) {
    const ready = getMusterReadyRoundSort(muster);
    if (ready === null) return true; // Legacy pending muster: allow GM fallback processing.
    return roundSortValue(clock) >= ready;
  }

  async function processArmyMusters(options = {}) {
    if (!requireOverviewScene()) return { processed: 0, skipped: 0, waiting: 0, failed: 0, rows: [] };
    if (!game.user.isGM) {
      ui.notifications.warn("Only the GM client can perform the privileged token-creation step for military musters.");
      return { processed: 0, skipped: 0, waiting: 0, failed: 0, rows: [] };
    }

    const onlyReady = options.onlyReady === true;
    const silent = options.silent === true;
    const clock = options.clock || getClock();
    let processed = 0, skipped = 0, waiting = 0, failed = 0;
    const rows = [];
    for (const characterToken of getCharacterTokens()) {
      ...
      if (armyMuster && armyMuster.status === "pending") {
        if (onlyReady && !isArmyMusterReady(armyMuster, clock)) {
          waiting++;
        } else if (getExistingArmyForCharacter(armyMuster.linkedCharacterId || character.characterId)) {
          skipped++;
        } else {
          try {
            ...
            const ownerUser = getUserByIdOrName(done.ownerUserId, done.ownerUserName);
            if (ownerUser && game.socket) {
              game.socket.emit(SOCKET_NAME, {
                type: "playerNotification",
                targetUserId: ownerUser.id,
                level: "info",
                message: `${done.armyName} has finished mustering and is now active at ${spawned.entry ? getTileName(spawned.entry) : done.currentTileName}.`
              });
            }
            await ChatMessage.create({
              speaker: ChatMessage.getSpeaker({ alias: "Crown Armies" }),
              whisper: ownerUser ? [ownerUser.id, ...ChatMessage.getWhisperRecipients("GM").map(user => user.id)] : ChatMessage.getWhisperRecipients("GM").map(user => user.id),
              content: `<h2>Army Muster Complete</h2><p><strong>Army:</strong> ${escapeHtml(done.armyName)}</p><p><strong>Strength:</strong> ${escapeHtml(Number(done.totalStrength || 0).toLocaleString())}</p><p><strong>Location:</strong> ${escapeHtml(spawned.entry ? getTileName(spawned.entry) : done.currentTileName)}</p><p>The army is now active and begins paying its normal round upkeep.</p>`
            });
    ...
    if (!silent) {
      ui.notifications.info(`Processed ${processed} military muster(s). ${waiting ? `${waiting} not ready. ` : ""}${skipped ? `${skipped} skipped. ` : ""}${failed ? `${failed} failed.` : ""}`);
      await ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ alias: "Crown Musters" }),
        content: `<h2>Military Musters Processed</h2><p><strong>Spawned:</strong> ${escapeHtml(processed)}</p><p><strong>Not Ready:</strong> ${escapeHtml(waiting)}</p><p><strong>Skipped:</strong> ${escapeHtml(skipped)}</p><p><strong>Failed:</strong> ${escapeHtml(failed)}</p>${rows.length ? `<ul>${rows.join("")}</ul>` : ""}`
      });
    }
    revealForCurrentPlayerPieces();
  ...
  function getEmbarkedArmyTokensForFleet(fleetToken) {
    ...
    return getArmyTokens().filter(token => {
      const army = getWorldPiece(token) || {};
      if (army.embarked !== true) return false;
      return String(army.embarkedFleetTokenId || "") === fleetId || String(army.transportFleetTokenId || "") === fleetId;
    });
  }
  ...
  async function updateCharacterEmbarkState(characterToken, destinationEntry, fleetToken, fleetPiece, { lock = true, clear = false, reason = "Embarked by navy" } = {}) {
    ...
    if (clear) {
      ...
      delete character.carriedByNavyTokenId;
      delete character.carriedByNavyName;
      delete character.fleetTokenId;
      delete character.fleetName;
      ...
      delete charPiece.carriedByNavyTokenId;
      delete charPiece.carriedByNavyName;
      delete charPiece.fleetTokenId;
      delete charPiece.fleetName;
    } else if (fleetToken) {
      character.embarkedFleetTokenId = fleetToken.document.id;
      character.embarkedFleetName = fleetPiece?.name || fleetToken.document.name;
    ...
    if (lock) {
      const lockRoundKey = getCurrentActionRoundKey();
      const lockTime = new Date().toISOString();
      charPiece.movementUsed = Math.max(Number(charPiece.movementUsed || 0), Number(charPiece.movementMax || 0));
      charPiece.movementLockedRoundKey = lockRoundKey;
      charPiece.movementLockedReason = reason;
      charPiece.movementLockedAt = lockTime;
      charPiece.movementLockedBy = game.user.name;
      character.movementLockedRoundKey = lockRoundKey;
      character.movementLockedReason = reason;
      character.movementLockedAt = lockTime;
      character.movementLockedBy = game.user.name;
    }
    await characterToken.document.setFlag(FLAG_SCOPE, WORLD_CHARACTER_KEY, character);
    if (characterToken.actor) await characterToken.actor.setFlag(FLAG_SCOPE, WORLD_CHARACTER_KEY, foundry.utils.deepClone(character));
  ...
  async function applyEmbarkArmyToFleet(armyToken, fleetToken, { actingUserId = game.user.id, actingUserName = game.user.name, selectedComposition = null } = {}) {
    const army = foundry.utils.deepClone(getWorldPiece(armyToken) || {});
    ...
    if (currentStrength > remaining) {
      const chosen = selectedComposition
        ? sanitizeEmbarkCompositionChoice(selectedComposition, composition, remaining)
        : await chooseEmbarkCompositionForCapacity(army, fleet, composition, remaining);
      if (!chosen) return null;
      composition = chosen.composition;
      embarkedStrength = chosen.total;
      surplus = Math.max(0, currentStrength - embarkedStrength);
    ...
    army.upkeep = calculateArmyUpkeep(composition, getSiegeEquipment(army));
    ...
    await armyToken.document.update({ x: armyPos.x, y: armyPos.y, hidden: false }, { animate: true, worldMovementBypass: true, bypassWorldMovementWatcher: true, embarkArmyBypass: true });
  ...
  async function embarkArmy() {
    ...
    if (!fleetId) return;
    const fleetToken = getFleetTokenById(fleetId);
    if (!fleetToken) { ui.notifications.warn("Could not find the selected navy."); return; }
    let selectedComposition = null;
    const transportInfo = getArmyTransportInfo(army, fleetToken);
    if (transportInfo.currentMen > transportInfo.remaining) {
      const originalComposition = getArmyComposition(army);
      const chosen = await chooseEmbarkCompositionForCapacity(army, getWorldPiece(fleetToken) || {}, originalComposition, transportInfo.remaining);
      if (!chosen) return;
      selectedComposition = chosen.composition;
    }
    if (!game.user.isGM) {
      const gm = findActiveGmForScene(canvas.scene?.id);
      if (!gm) { ui.notifications.warn("No active GM online to embark this army."); return; }
      game.socket.emit(SOCKET_NAME, { type: "embarkArmyRequest", targetGmId: gm.id, sceneId: canvas.scene?.id, requesterUserId: game.user.id, requesterUserName: game.user.name, armyTokenId: armyToken.document.id, armyTokenName: armyToken.document.name, fleetTokenId: fleetId, selectedComposition });
      ui.notifications.info(`Embark Army request sent to GM ${gm.name}.`);
      return;
    }
    await applyEmbarkArmyToFleet(armyToken, fleetToken, { actingUserId: game.user.id, actingUserName: game.user.name, selectedComposition });
  }
  ...
  async function handleEmbarkArmyRequest(message) {
    ...
    try { await applyEmbarkArmyToFleet(armyToken, fleetToken, { actingUserId: message.requesterUserId, actingUserName: message.requesterUserName || "Player", selectedComposition: message.selectedComposition || null }); }
    catch (err) {
  ...
  async function applyDisembarkArmyFromFleet(armyToken, destinationEntry, { actingUserId = game.user.id, actingUserName = game.user.name } = {}) {
    ...
    delete army.carriedByNavyTokenId;
    delete army.carriedByNavyName;
    delete army.fleetTokenId;
    delete army.fleetName;
    ...
    fleet.transportUsed = Array.isArray(fleet.embarkedArmies) ? fleet.embarkedArmies.reduce((sum, item) => sum + Math.max(0, Number(item.strength || item.troops || 0)), 0) : 0;
    fleet.movementUsed = Math.max(Number(fleet.movementUsed || 0), Number(fleet.movementMax || 0));
    fleet.movementLockedRoundKey = getCurrentActionRoundKey();
    fleet.movementLockedReason = "Disembarked an army this turn.";
    fleet.movementLockedAt = now;
    fleet.movementLockedBy = actingUserName;
    fleet.updatedAt = now;
    fleet.updatedBy = game.user.name;
    await saveWorldPiece(fleetToken, fleet);
    ...
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ alias: "Crown Embarkation" }),
      content: `<h2>Army Disembarked</h2><p><strong>Army:</strong> ${escapeHtml(army.name || armyToken.document.name)}</p><p><strong>From Navy:</strong> ${escapeHtml(oldFleetName)}</p><p><strong>Landing:</strong> ${escapeHtml(getTileName(destinationEntry))}</p><p><strong>Troops:</strong> ${escapeHtml(getArmyCurrentStrength(army).toLocaleString())}</p><p>Disembarking consumed the army, commander, and navy's movement for this turn. None can move again until movement resets.</p>`
    });
    revealForCurrentPlayerPieces();
    return { army, fleet };
  }
  ...
  async function moveEmbarkedArmiesWithFleet(fleetToken, fleetPiece, destinationEntry) {
    ...
    for (const armyToken of armies) {
      const army = foundry.utils.deepClone(getWorldPiece(armyToken) || {});
      if (army.embarked !== true || (String(army.embarkedFleetTokenId || army.transportFleetTokenId || "") !== String(fleetToken.document.id))) continue;
      army.previousTileId = army.currentTileId || "";
      army.previousTileName = army.currentTileName || "";
      army.currentTileId = getTileId(destinationEntry);
      army.currentTileName = getTileName(destinationEntry);
      ...
      await saveWorldPiece(armyToken, army);
      const pos = getTokenTopLeftForTileSlot(armyToken, destinationEntry);
      await armyToken.document.update({ x: pos.x, y: pos.y, hidden: false }, { animate: false, worldMovementBypass: true, bypassWorldMovementWatcher: true, fleetCargoBypass: true });
  ...
  async function editSelectedArmy() {
    ...
    const linkedCharacter = !isFleet ? getCharacterTokenById(piece.linkedCharacterId || piece.commanderCharacterId) : null;
    const siegeSummary = !isFleet && linkedCharacter
      ? getAvailableSiegeCapacity(linkedCharacter, "", token.document.id)
      : { tier: 3, total: 999, used: 0, available: 999 };
    const siegeEquipment = !isFleet ? getSiegeEquipment(piece) : {};
    const compositionHtml = isFleet
      ? NAVY_SHIP_TYPES.map(ship => `<div class="form-group"><label>${escapeHtml(ship.label)}</label><input type="number" name="ship_${escapeHtml(ship.key)}" value="${escapeHtml(composition[ship.key] || 0)}" min="0" step="1" style="width:100%;" /><p class="notes">Round upkeep: ${escapeHtml(ship.gold)} Gold + ${escapeHtml(ship.food)} Food per 5 ships</p></div>`).join("")
    ...
    const result = await new Promise(resolve => {
      new Dialog({
        ...
        content: `<form>
          ...
          ${isFleet ? "" : `<hr><h3>Siege Equipment</h3><p class="notes">House Siege Capacity: ${escapeHtml(siegeSummary.available)} available / ${escapeHtml(siegeSummary.total)} total. ${escapeHtml(getSiegeInfrastructureSummary(siegeSummary.tier))}</p><div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;">${buildSiegeEquipmentInputs(siegeSummary, siegeEquipment)}</div><div class="form-group"><label>Siege Turns</label><input type="number" name="siegeTurns" value="${escapeHtml(piece.siegeTurns || 1)}" min="1" step="1" style="width:100%;" /></div>`}
        ...
        buttons: { save: { label: `Save ${isFleet ? "Navy" : "Army"}`, callback: html => {
          const form = html[0].querySelector("form");
          resolve({ forceName: String(form.forceName.value || "").trim(), composition: isFleet ? readNavyCompositionForm(form) : readArmyCompositionForm(form), siegeEquipment: isFleet ? {} : readSiegeEquipmentForm(form), siegeTurns: isFleet ? Number(piece.siegeTurns || 1) : Math.max(1, Number(form.siegeTurns.value || 1)), followCharacter: form.followCharacter.checked, detached: form.detached.checked });
        }}, cancel: { label: "Cancel", callback: () => resolve(null) } },
        default: "save"
      }, { width: 820, height: 860, resizable: true }).render(true);
    });
    if (!result) return;
    if (isFleet) {
      const linkedCharacterForFleet = getCharacterTokenById(piece.linkedCharacterId || piece.commanderCharacterId);
      if (linkedCharacterForFleet) {
        try { validateNavyShipClassUnlocks(linkedCharacterForFleet, result.composition); }
        catch (err) { ui.notifications.error(err.message || "This House lacks the naval infrastructure for that ship class."); return; }
      }
    } else {
      try { validateSiegeEquipmentSelection(result.siegeEquipment, siegeSummary); }
      catch (err) { ui.notifications.error(err.message || "Invalid siege equipment selection."); return; }
    }
    ...
    if (isFleet) {
      ...
      piece.siegeEquipment = result.siegeEquipment;
      piece.siegeCapacityUsed = getSiegeEquipmentCapacityUsed(result.siegeEquipment);
      piece.siegeUpkeep = calculateSiegeEquipmentUpkeep(result.siegeEquipment);
      piece.siegeEngines = getSiegeEquipmentCount(result.siegeEquipment);
      piece.upkeep = calculateArmyUpkeep(result.composition, result.siegeEquipment);
    }
  ...
  function getFortificationLevelForHouse(house = {}, clock = getClock()) {
    ...
    const state = getActiveBuildingLineState(house, clock).get("watchtower");
    if (state?.level) return Math.max(0, Math.min(3, Number(state.level.level || 0)));
    const built = getActiveBuiltBuildings(house, clock);
  ...
  function getSiegeSettlementKey(house = {}, clock = getClock()) {
    const activeLevel = getActiveBuildingSlotCount(house, clock);
    const level = Math.max(0, Math.min(4, Number(activeLevel || 0)));
    return ["ruins", "village", "holdfast", "town", "city"][level] || "ruins";
  }

  function getSiegeSettlementLevel(key) {
    return Math.max(0, ["ruins", "village", "holdfast", "town", "city"].indexOf(String(key || "").toLowerCase()));
  }

  function siegeSettlementLabel(key) {
    const labels = { ruins: "Ruins", village: "Village", holdfast: "Holdfast", town: "Town", city: "City" };
    return labels[String(key || "").toLowerCase()] || titleCase(key);
  }

  function getSiegeDcDetails(house = {}, clock = getClock()) {
    const settlementKey = getSiegeSettlementKey(house, clock);
    const settlementLevel = getSiegeSettlementLevel(settlementKey);
  ...
  function getSiegeEngineEffects(equipment = {}, settlementKey = "ruins", fortLevel = 0) {
    ...
    for (const engine of SIEGE_ENGINE_TYPES) {
      const count = Math.max(0, Math.floor(Number(equipment?.[engine.key] || 0)));
      if (!count) continue;
      let eachBonus = Number(engine.baseBonus || 0);
      let reason = `base +${engine.baseBonus || 0}%`;
      if (engine.key === "Battering Ram" && fortLevel > 0) { eachBonus += 2; reason += ", +2% fortified target"; }
      if (engine.key === "Onager" && settlementLevel >= 2) { eachBonus += 2; reason += ", +2% developed target"; }
      if (engine.key === "Trebuchet" && (settlementLevel >= 3 || fortLevel >= 2)) { eachBonus += 5; reason += ", +5% major/fortified target"; }
      rawBonus += count * eachBonus;
      if (engine.key === "Ballista") casualtyReduction += count * Number(engine.casualtyReduction || 1);
      details.push(`${count} ${engine.label}${count === 1 ? "" : "s"}: +${count * eachBonus}% (${reason})`);
    }
    const bonus = Math.min(MAX_SIEGE_ENGINE_BONUS, rawBonus);
    casualtyReduction = Math.min(MAX_BALLISTA_CASUALTY_REDUCTION, casualtyReduction);
    return { rawBonus, bonus, casualtyReduction, details, capacity: getSiegeEquipmentCapacityUsed(equipment) };
  }
  ...
  function getOpposingArmiesOnTileForRaid(entry, attackerPiece = {}, attackingOwnerId = "") {
    const tileId = String(getTileId(entry) || "");
    const attackerHouse = normalize(getConqueringHouseName(attackerPiece, attackingOwnerId, attackerPiece.ownerUserName || attackerPiece.playerOwnerUserName || ""));
    return getArmyTokens().filter(token => {
      const army = getWorldPiece(token) || {};
  ...
  function getSiegeOutcome(roll, chance) {
    ...
    if (miss <= 10) return { key: "foothold", label: "Foothold", success: false, foothold: true, casualtyPercent: 5, text: "The settlement holds, but attackers gain +10% on the next storm attempt if the siege continues." };
  ...
  function calculateSiegeCasualties(currentStrength, outcome, casualtyReduction = 0) {
    const before = Math.max(0, Math.floor(Number(currentStrength || 0)));
    const basePercent = Math.max(0, Number(outcome?.casualtyPercent || 0));
    const reduction = Math.min(basePercent, Math.max(0, Number(casualtyReduction || 0)));
    const percent = Math.max(0, basePercent - reduction);
    const lost = before > 0 && percent > 0 ? Math.max(1, Math.floor(before * percent / 100)) : 0;
    return { before, basePercent, reduction, percent, lost, after: Math.max(0, before - lost) };
  }
  ...
  function getConqueringHouseName(attackerPiece = {}, actingUserId = "", actingUserName = "") {
    ...
    return String(attackerPiece.allegiance || attackerPiece.house || attackerPiece.faction || actingUserName || "").trim();
  }

  function isGenericNeutralLocalHouse(value = "") {
    const key = normalize(value || "");
    return !key ||
      ["neutral", "npc", "none", "unaligned", "unassigned"].includes(key) ||
      key.startsWith("neutral ") ||
      key.startsWith("npc ");
  }
  ...
  function applyProvinceTransferData(worldTile = {}, house = {}, { houseName = "", actingUserId = "", actingUserName = "", source = "Transfer", actorName = "", metadata = {} } = {}) {
    const nextWorld = foundry.utils.deepClone(worldTile || {});
    const nextHouse = foundry.utils.deepClone(house || {});
    const allegiance = String(houseName || actingUserName || "").trim();
    const currentLocalHouse = String(nextHouse.house || nextWorld.house || nextWorld.localHouse || nextWorld.owner || "").trim();
    ...
    nextWorld.playerOwnerUserName = actingUserName || "";
    nextWorld.ownershipType = "Player";
    nextWorld.ownerType = "Player";
    nextWorld.allegiance = allegiance;
    nextWorld.publicOwnerLabel = allegiance;
    if (genericLocalHouse) {
      nextWorld.owner = allegiance;
      nextWorld.house = allegiance;
      nextWorld.localHouse = allegiance;
    }

    nextHouse.ownerUserId = actingUserId || "";
    nextHouse.ownerUserName = actingUserName || "";
    nextHouse.playerOwnerUserId = actingUserId || "";
    nextHouse.playerOwnerUserName = actingUserName || "";
    nextHouse.ownershipType = "Player";
    nextHouse.ownerType = "Player";
    nextHouse.allegiance = allegiance;
    ...
    nextHouse.lastOwnershipChange = audit;
    nextHouse.updatedAt = now;
    nextHouse.updatedBy = game.user.name;

    return { worldTile: nextWorld, house: nextHouse, allegiance, houseName: allegiance, controllerId: actingUserId || "", controllerName: actingUserName || "" };
  }

  async function replaceProvincePoliticalFlags(entry, worldTile = {}, house = {}) {
    const doc = entry?.drawing?.document;
    if (!doc) throw new Error("Province transfer is missing its drawing document.");

    // Replace the complete flag objects. This avoids stale nested political fields
    // surviving from older Neutral / Unaligned data after a transfer.
    await doc.unsetFlag(FLAG_SCOPE, WORLD_TILE_KEY);
    await doc.setFlag(FLAG_SCOPE, WORLD_TILE_KEY, foundry.utils.deepClone(worldTile || {}));
    await doc.unsetFlag(FLAG_SCOPE, HOUSE_KEY);
    await doc.setFlag(FLAG_SCOPE, HOUSE_KEY, foundry.utils.deepClone(house || {}));
  }

  async function verifyProvinceAllegiance(entry, expectedAllegiance = "", expectedControllerId = "", expectedControllerName = "") {
    const worldTile = getWorldTile(entry.drawing) || {};
    const house = getHouseData(entry.drawing) || {};
    const tileAllegiance = String(worldTile.allegiance || "").trim();
    const houseAllegiance = String(house.allegiance || "").trim();
    const savedControllerId = String(getTileOwnerUserId(worldTile, house) || "").trim();
    const savedControllerName = String(getTileOwnerUserName(worldTile, house) || "").trim();
    if (expectedAllegiance && normalize(tileAllegiance) !== normalize(expectedAllegiance)) throw new Error(`Province transfer did not persist world-tile allegiance. Expected ${expectedAllegiance}, found ${tileAllegiance || "blank"}.`);
    if (expectedAllegiance && normalize(houseAllegiance) !== normalize(expectedAllegiance)) throw new Error(`Province transfer did not persist House allegiance. Expected ${expectedAllegiance}, found ${houseAllegiance || "blank"}.`);
    if (expectedControllerId && savedControllerId && String(savedControllerId) !== String(expectedControllerId)) throw new Error(`Province transfer persisted the wrong controller ID (${savedControllerId}).`);
    if (expectedControllerName && savedControllerName && normalize(savedControllerName) !== normalize(expectedControllerName)) throw new Error(`Province transfer persisted the wrong controller name (${savedControllerName}).`);
  ...
  async function transferProvinceToHouse(entry, attackerPiece, actingUserId, actingUserName, source = "Transfer", metadata = {}) {
    ...
    try {
      await replaceProvincePoliticalFlags(entry, result.worldTile, result.house);
  ...
  async function resolveSiegeStorm({ token, piece, entry, options, actingUserId = game.user.id, actingUserName = game.user.name } = {}) {
    ...
    const engineEffects = getSiegeEngineEffects(equipment, dc.settlementKey, dc.fortLevel);
    const martialBonus = martial * 2;
    const manpowerBonus = getSiegeManpowerBonus(strength);
    const engineBonus = engineEffects.bonus;
    const durationBonus = getSiegeDurationBonus(result.siegeTurns);
    const footholdBonus = Number(piece.footholdBonus || 0);
    const rawChance = 50 + martialBonus + manpowerBonus + engineBonus + durationBonus + footholdBonus - dc.total;
    ...
    const casualties = calculateSiegeCasualties(strength, outcome, engineEffects.casualtyReduction);
    updatedPiece.siegeEquipment = equipment;
    updatedPiece.siegeCapacityUsed = getSiegeEquipmentCapacityUsed(equipment);
    updatedPiece.siegeUpkeep = calculateSiegeEquipmentUpkeep(equipment);
    updatedPiece.siegeEngines = getSiegeEquipmentCount(equipment);
    ...
    const engineDetailText = engineEffects.details.length ? engineEffects.details.join("; ") : "No siege equipment";
    await ChatMessage.create({
  ...
  function getRaidDefense(house = {}) {
    ...
    return { base: 50, fortLevel, barracksLevel, fortBonus, barracksBonus, total: 50 + fortBonus + barracksBonus };
  }

  function getRaidPercent(roll, adjustedRoll, defense) {
    if (Number(roll) === 100) return 1;
    const margin = Number(adjustedRoll || 0) - Number(defense || 0);
    if (margin < 0) return 0;
    if (margin <= 20) return 0.25;
    if (margin <= 40) return 0.50;
    return 0.75;
  }

  async function creditRaidLootToRealm(entries = [], loot = {}, reason = "Raid loot") {
    const landEntries = entries.filter(entry => entry?.drawing?.document && !isSeaByTile(entry.tile));
    if (!landEntries.length) throw new Error("No controlled realm holdings were found to receive raid loot.");
    const normalized = normalizeResourceMap(loot);
    for (const [resource, amountRaw] of Object.entries(normalized)) {
      const amount = Math.max(0, Number(amountRaw || 0));
      if (!amount) continue;
      const share = cleanResourceNumber(amount / landEntries.length);
      let assigned = 0;
      for (let i = 0; i < landEntries.length; i++) {
        const delta = i === landEntries.length - 1 ? cleanResourceNumber(amount - assigned) : share;
        assigned = cleanResourceNumber(assigned + delta);
        await applyResourceDeltaToTile(landEntries[i], { [resource]: delta }, reason);
      }
    }
  }

  async function resolveRaidTerritory({ token, piece, entry, actingUserId = game.user.id, actingUserName = game.user.name } = {}) {
    if (!token || !piece || !entry?.tile) throw new Error("Missing army or target tile for raid.");
    const roundKey = getCurrentActionRoundKey();
    const strength = Math.max(0, Math.floor(Number(piece.strengthCurrent || piece.totalStrength || 0)));
    if (strength < 500) throw new Error("A raid requires at least 500 troops.");
    ...
    const ownerId = getTileOwnerUserId(entry.tile, house);
    if (!ownerId && !defenderHouseKey) throw new Error(`${getTileName(entry)} is unaligned and has no enemy realm to raid.`);
    if (ownerId && String(ownerId) === String(attackerOwnerId || "")) throw new Error(`${getTileName(entry)} is controlled by this army's owner.`);

    // Hard stop: raids never resolve while ANY opposing army is physically present.
    const defendingArmies = getOpposingArmiesOnTileForRaid(entry, piece, attackerOwnerId);
    if (defendingArmies.length) throw new Error(`Cannot raid ${getTileName(entry)} while an opposing army is present. Defeat or remove the defending army first.`);

    const linkedCommander = getCharacterTokenById(piece.linkedCharacterId || piece.commanderCharacterId);
    const martial = Number(piece.commanderMartial || (linkedCommander ? getCharacterMartialValue(linkedCommander) : 0) || 0);
    const defense = getRaidDefense(house);
    const rollObj = await new Roll("1d100").evaluate({ async: true });
    const roll = Number(rollObj.total || 0);
    ...
    const defendersAtCommit = getOpposingArmiesOnTileForRaid(entry, piece, attackerOwnerId);
    ...
    if (hasAnyResources(loot)) await creditRaidLootToRealm(receiverEntries, loot, `Loot from raid of ${getTileName(entry)}`);

    const updatedPiece = foundry.utils.deepClone(piece);
    updatedPiece.lastRaidRoundKey = roundKey;
    updatedPiece.lastRaidAt = new Date().toISOString();
    updatedPiece.lastRaidTarget = getTileName(entry);
    updatedPiece.movementUsed = Math.max(Number(updatedPiece.movementUsed || 0), Number(updatedPiece.movementMax || 0));
    ...
    ui.notifications.info(`${resultLabel}: ${resourceMapToText(loot, "nothing stolen")}.`);
    return { roll, adjusted, percent, defense, loot };
  }

  async function requestGmRaidTerritory({ token, piece, entry }) {
    const gm = findActiveGmForScene(canvas.scene?.id);
    if (!gm) { ui.notifications.warn("No active GM online to resolve the raid."); return false; }
    game.socket.emit(SOCKET_NAME, { type: "raidTerritoryRequest", targetGmId: gm.id, sceneId: canvas.scene?.id, requesterUserId: game.user.id, requesterUserName: game.user.name, tokenId: token.document.id, drawingId: entry.drawing.document.id, tileId: getTileId(entry), tileName: getTileName(entry) });
    ui.notifications.info(`Raid request sent to GM ${gm.name}.`);
    return true;
  }

  async function raidTerritory() {
    if (!requireOverviewScene()) return;
    const selected = canvas.tokens.controlled.filter(token => normalize(getWorldPiece(token)?.pieceType) === "army");
    if (selected.length !== 1) { ui.notifications.warn("Select exactly one army token on the territory you want to raid."); return; }
    const token = selected[0];
    const piece = foundry.utils.deepClone(getWorldPiece(token));
    if (!canUserControlWorldPiece(token, piece)) { ui.notifications.warn("You can only raid with an army you control."); return; }
    const lockedReason = strategicMovementLockReason(piece);
    if (lockedReason) { ui.notifications.warn(lockedReason); return; }
  ...
  async function handleRaidTerritoryRequest(message) {
    if (!game.user.isGM || (message.targetGmId && String(message.targetGmId) !== String(game.user.id))) return;
    try {
      const token = canvas.tokens.get(String(message.tokenId || ""));
      if (!token) throw new Error("Raiding army token could not be found.");
      const piece = foundry.utils.deepClone(getWorldPiece(token));
      const entry = getWorldTileEntries().find(candidate => candidate.drawing.document.id === message.drawingId || String(getTileId(candidate)) === String(message.tileId));
      if (!entry?.tile) throw new Error(`Could not find raid target ${message.tileName || message.tileId}.`);
      await resolveRaidTerritory({ token, piece, entry, actingUserId: message.requesterUserId, actingUserName: message.requesterUserName || "Player" });
    } catch (err) {
      console.error("Crown Overview raid request failed:", err, message);
      const requester = game.users.get(String(message.requesterUserId || ""));
      if (requester && game.socket) game.socket.emit(SOCKET_NAME, { type: "playerNotification", targetUserId: requester.id, level: "error", message: `Raid failed: ${err.message || err}` });
      ui.notifications.error(`Raid request failed: ${err.message || err}`);
    }
  }
    ...
    const result = await new Promise(resolve => {
      new Dialog({
        ...
        content: `<form>
          ...
          <div style="padding:8px;margin-bottom:10px;border:1px solid #777;border-radius:6px;">
            <strong>Attached Siege Equipment:</strong> ${escapeHtml(siegeEquipmentText(equipment))}<br>
            <strong>Capacity Used:</strong> ${escapeHtml(engineEffects.capacity)}<br>
            <strong>Storm Bonus:</strong> +${escapeHtml(engineEffects.bonus)}%${engineEffects.rawBonus > engineEffects.bonus ? ` (raw +${escapeHtml(engineEffects.rawBonus)}%, capped)` : ""}<br>
...
function clearCharacterTransportFlags(data = {}) {
  const updated = foundry.utils.deepClone(data || {});
  updated.embarked = false;
  delete updated.embarkedFleetTokenId;
  delete updated.embarkedFleetName;
  delete updated.embarkedArmyTokenId;
  delete updated.transportFleetTokenId;
  delete updated.transportFleetName;
  delete updated.carriedByFleetTokenId;
  delete updated.carriedByFleetName;
  delete updated.carriedByNavyTokenId;
  delete updated.carriedByNavyName;
  delete updated.fleetTokenId;
  delete updated.fleetName;
  return updated;
}

async function normalizeCharacterEmbarkState(token, piece = getWorldPiece(token) || {}, character = getCharacterDataFromToken(token) || {}) {
  const flagged = Boolean(
    piece.embarked || piece.embarkedFleetTokenId || piece.transportFleetTokenId || piece.carriedByFleetTokenId || piece.carriedByNavyTokenId ||
    character.embarked || character.embarkedFleetTokenId || character.transportFleetTokenId || character.carriedByFleetTokenId || character.carriedByNavyTokenId
  );
  if (!flagged) return { piece, character, embarkedAtSea: false, repaired: false };

  // Position wins over stale transport metadata. A character physically located on a land tile
  // cannot still be treated as embarked just because an old fleet id survived in flags.
  const actualEntry = getCurrentTileEntryForToken(token, piece);
  if (actualEntry?.tile && !isSeaTile(actualEntry.tile)) {
    const cleanPiece = clearCharacterTransportFlags(piece);
    const cleanCharacter = clearCharacterTransportFlags(character);
    await token.document.setFlag(FLAG_SCOPE, WORLD_CHARACTER_KEY, cleanCharacter);
    if (token.actor) await token.actor.setFlag(FLAG_SCOPE, WORLD_CHARACTER_KEY, foundry.utils.deepClone(cleanCharacter));
    await saveWorldPiece(token, cleanPiece);
    return { piece: cleanPiece, character: cleanCharacter, embarkedAtSea: false, repaired: true };
  }

  // If the character is genuinely on a sea tile and still carries embark metadata, keep the block.
  ...
  async function applyDiplomaticTakeover({ token, piece, entry, actingUserId, actingUserName }) {
    ...
    let character = getCharacterDataFromToken(token) || {};
    ...
    let worldTile = foundry.utils.deepClone(entry.tile);
    let house = foundry.utils.deepClone(entry.drawing.document.getFlag(FLAG_SCOPE, HOUSE_KEY) || {});
    const currentOwnerId = getTileOwnerUserId(worldTile, house);
    ...
    if (success) {
      ...
      // IMPORTANT: persist the transferred clones rather than saving the stale
      // pre-transfer local objects back over them.
      worldTile = transfer.worldTile;
    ...
    house.updatedBy = game.user.name;
    if (success) {
      await replaceProvincePoliticalFlags(entry, worldTile, house);
    } else {
      await entry.drawing.document.setFlag(FLAG_SCOPE, WORLD_TILE_KEY, worldTile);
      await entry.drawing.document.setFlag(FLAG_SCOPE, HOUSE_KEY, house);
    }
  ...
  async function handleDiplomacyRequest(message) {
    ...
    try {
      ...
      const recipients = [message.requesterUserId, ...game.users.contents.filter(user => user.isGM).map(user => user.id)].filter(Boolean);
      await ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ alias: "Crown Diplomacy" }),
        whisper: recipients,
        content: `<h2>Diplomatic Takeover Blocked</h2><p><strong>Character:</strong> ${escapeHtml(token.document.name || message.tokenName || "Unknown")}</p><p><strong>Reason:</strong> ${escapeHtml(reason)}</p>`
      });
    }
  }
  ...
  function getCharacterTurnStatusForMenu(token) {
    ...
    const armyComposition = linkedArmy ? armyCompositionText(getArmyComposition(linkedArmyPiece)) : "None";
    const armyUpkeep = linkedArmy ? resourceMapToText(linkedArmyPiece.upkeep || calculateArmyUpkeep(getArmyComposition(linkedArmyPiece), getSiegeEquipment(linkedArmyPiece))) : "None";
    const influence = getInfluenceSummaryForEntries(getManpowerEntriesForCharacter(token));
  ...
  async function selectCharacterTokenFromMenu(token, { action = "select" } = {}) {
    ...
    if (action === "build") await buildOnCurrentTile();
  }
  ...
  async function selectArmyFromCharacterMenu(characterToken, linkedArmy, action = "select") {
    ...
    if (action === "raid") await raidTerritory();
  }
  ...
  async function characterMoveMenu() {
    ...
    const rows = characters.map(status => {
      ...
      return `<div style="display:grid;grid-template-columns:minmax(180px,1.15fr) minmax(360px,2.2fr) minmax(250px,auto);gap:10px;align-items:center;padding:10px;border:1px solid #777;border-radius:6px;margin-bottom:8px;">
        <div><strong>${escapeHtml(name)}</strong><br><span class="notes">${escapeHtml(houseName)}</span><br><strong>Current Tile:</strong> ${escapeHtml(status.location)}</div>
        <div style="font-size:12px;line-height:1.45;">
          <strong>Stats:</strong> ${escapeHtml(status.statsText)}<br>
          <strong>Secondary:</strong> ${escapeHtml(status.secondaryStatsText)}<br>
          <strong>House Influence:</strong> ${escapeHtml(status.influenceText)}<br>
...
function getPoliticalOverlayMode() {
  try {
    const value = String(localStorage.getItem(POLITICAL_OVERLAY_STATE_KEY) || "off").trim().toLowerCase();
    return value === "on" ? "on" : "off";
  } catch (err) {
    const value = String(globalThis[POLITICAL_OVERLAY_STATE_KEY] || "off").trim().toLowerCase();
    return value === "on" ? "on" : "off";
  }
}

function setPoliticalOverlayMode(mode = "off") {
  const value = String(mode || "off").trim().toLowerCase() === "on" ? "on" : "off";
  try { localStorage.setItem(POLITICAL_OVERLAY_STATE_KEY, value); } catch (err) {}
  globalThis[POLITICAL_OVERLAY_STATE_KEY] = value;
}
...
function getRegionHue(regionName = "") {
  const region = normalize(regionName || "");
  if (region.includes("reach")) return 118;
  if (region.includes("north")) return 205;
  if (region.includes("river")) return 188;
  if (region.includes("vale")) return 220;
  if (region.includes("west")) return 4;
  if (region.includes("storm")) return 162;
  if (region.includes("crown")) return 286;
  if (region.includes("dorne")) return 28;
  if (region.includes("iron")) return 228;
  if (region.includes("island")) return 228;
  return 44;
}

function drawTileShapeWithStyle(graphics, drawing, fillColor, fillAlpha = 0, lineColor = fillColor, lineAlpha = 0.8, lineWidth = 2) {
  const doc = drawing.document;
  const shape = doc.shape || {};
  const type = normalizeShapeType(shape.type);
  const x = Number(doc.x || 0);
...
function getPoliticalOverlayPlayerPalette() {
  ...
  return [
    ...
    0x47b7d1, 0xb65fd2, 0x6f8fd6, 0xd05f66, 0x51a88f, 0x9d6bd6,
    0xc17f3b, 0x648f45, 0xb14e6d, 0x4f78a8, 0x9b7a39, 0x4e9f7e
  ];
}
...
async function togglePoliticalOverlay() {
  ...
  function buildCategoryOrder(options = []) {
    const preferred = ["Economy", "Infrastructure", "Military", "Social", "Legacy"];
    const present = new Set(options.map(option => option.category || option.group || "Buildings"));
    return preferred.filter(category => present.has(category)).concat([...present].filter(category => !preferred.includes(category)));
  }

  function conciseBuildingOptionsHtml(options = [], category = "", selectedValue = "") {
    const rows = options.filter(option => !category || (option.category || option.group) === category);
  ...
  function getDevelopmentEconomyBonus(house = {}, clock = getClock()) {
    const activeLevel = getActiveBuildingSlotCount(house, clock);
    const level = Math.max(0, Math.min(4, Number(activeLevel || 0)));
  ...
  function addRoundsToClockData(clock = getClock(), rounds = 1) {
    const base = clock || getDefaultClock();
    const step = Math.max(1, Math.floor(Number(rounds || 1)));
    const totalIndex = Number(base.roundIndex ?? 0) + step;
    const nextYear = Number(base.year ?? 100) + Math.floor(totalIndex / ROUND_ORDER.length);
    const nextIndex = ((totalIndex % ROUND_ORDER.length) + ROUND_ORDER.length) % ROUND_ORDER.length;
    const nextRound = ROUND_ORDER[nextIndex] || ROUND_ORDER[0];
    return { year: nextYear, roundIndex: nextIndex, season: nextRound.season, round: nextRound.round, version: MODULE_VERSION, updatedAt: new Date().toISOString(), updatedBy: game.user.name };
  }

  function getNextRoundActivation(clock = getClock(), constructionRounds = 1) {
    const next = addRoundsToClockData(clock || getDefaultClock(), constructionRounds);
    return {
      ...
      constructionRounds: Math.max(1, Math.floor(Number(constructionRounds || 1)))
    };
  }
  ...
  async function applyBuildToTile({ token, piece, entry, building, builderUserId, builderUserName }) {
    ...
    const mineFoodConflict = getMineFoodBuildingConflict(house, building);
    ...
    ui.notifications.info(`${building} ${isUpgrade ? "upgrade" : "construction"} started in ${worldTile.name || "selected tile"}. Bonuses unlock on ${activation.activeFromDateLabel}.`);
    return true;
  }
  ...
  function registerSocketHandlers() {
    ...
    game.socket.on(SOCKET_NAME, async message => {
      if (!message) return;
      if (message.type === "playerNotification") {
  ...
  async function buildOnCurrentTile() {
    ...
    const dateLabel = getDateLabel(clock);
    const categories = buildCategoryOrder(catalogOptions);
    const firstCategory = categories.find(category => catalogOptions.some(option => (option.category || option.group) === category && !option.disabled)) || categories[0] || "Buildings";
        ...
        buttons: {
          build: {
            ...
            callback: html => {
              ...
              const option = catalogOptions.find(row => String(row.value) === value);
              if (option?.disabled) { ui.notifications.warn(option.lockedReason || "This building is locked."); resolve(null); return; }
              resolve({ building: value });
            }
          },
          cancel: { label: "Cancel", callback: () => resolve(null) }
        },
        ...
        render: html => {
          ...
          const activateCategory = category => {
            for (const button of buttons) {
              const active = String(button.dataset.buildingCategory || "") === String(category || "");
              button.style.fontWeight = active ? "700" : "400";
              button.style.outline = active ? "2px solid rgba(180,145,90,0.8)" : "none";
            }
            const rows = catalogOptions.filter(option => (option.category || option.group) === category);
            const preferred = rows.find(option => !option.disabled) || rows[0] || null;
            select.innerHTML = conciseBuildingOptionsHtml(catalogOptions, category, preferred?.value || "");
            select.disabled = !rows.length;
            renderDetail();
          };
          buttons.forEach(button => button.addEventListener("click", () => activateCategory(String(button.dataset.buildingCategory || ""))));
    ...
    const selectedRule = getBuildingRule(details.building, house);
    const selectedReq = Number(selectedRule.statecraftRequired || 0);
    const selectedStatecraft = getPieceStatecraft(piece);
    if (!game.user.isGM && selectedStatecraft !== null && selectedReq > 0 && selectedStatecraft < selectedReq) {
      ui.notifications.warn(`${piece.name || token.document.name} needs Statecraft ${selectedReq} to build ${details.building}, but has ${selectedStatecraft}.`);
      return;
    }
...
async function assignHouse() {
  ...
  const existingState = getBuildingLineState(existing);
  ...
  for (const line of BUILDING_LINES) {
    const group = line.group || "Buildings";
    if (!groupedLines.has(group)) groupedLines.set(group, []);
    groupedLines.get(group).push(line);
  }
  const buildingEditorHtml = isSea ? "" : [...groupedLines.entries()].map(([group, lines]) => {
    const rows = lines.map(line => {
      const current = existingState.get(line.key);
      const options = [`<option value="">None</option>`, ...line.levels.map(level => {
        const selected = current?.level?.name === level.name ? "selected" : "";
        const effect = getBuildingDisplayEffect(line, level, existing);
        return `<option value="${escapeHtml(level.name)}" ${selected}>Tier ${level.level} — ${escapeHtml(level.name)} (${escapeHtml(effect)})</option>`;
      })].join("");
      return `<div style="display:grid;grid-template-columns:220px 1fr;gap:8px;align-items:center;margin:6px 0;">
        <label for="line_${escapeHtml(line.key)}"><strong>${escapeHtml(line.label)}</strong></label>
        <select name="line_${escapeHtml(line.key)}" style="width:100%;">${options}</select>
      </div>`;
    }).join("");
    return `<div style="padding:10px;margin-top:10px;border:1px solid #777;border-radius:6px;">
      <h3 style="margin:0 0 8px 0;">${escapeHtml(group)}</h3>
      ${rows}
    </div>`;
  }).join("");

  const developmentSection = isSea ? `<hr><h2>Sea Tile</h2><div style="padding:10px;border:1px solid #777;border-radius:6px;margin-bottom:10px;"><strong>Sea Terrain</strong><br><span style="font-size:12px;opacity:0.85;">Development, population, and built buildings do not apply to sea tiles.</span></div>` : `<hr><h2>Development</h2><div style="padding:8px;border:1px solid #777;border-radius:6px;margin-bottom:10px;"><strong>Current Development:</strong> ${escapeHtml(currentBuildingCount + " — " + currentDevelopment.label)}<br><strong>Building Slots Used:</strong> ${escapeHtml(currentBuildingCount)} / 4</div><div class="form-group"><label>Population</label><input type="number" name="population" value="${escapeHtml(existing.population ?? "")}" style="width:100%;" /><p class="notes">Population rerolls automatically whenever the number of active building lines changes.</p></div>`;
  ...
  const content = `<form>
    ...
    ${isSea ? "" : `<hr><h2>Buildings</h2><p class="notes">GM Override: choose any tier directly. This bypasses Statecraft so you can test or set up holdings quickly. A tile may still only have <strong>4 active building lines</strong>.</p>${buildingEditorHtml}`}
  </form>`;
  ...
  new Dialog({
    ...
    buttons: {
      save: {
        ...
        callback: async html => {
          const form = html[0].querySelector("form");
          const ownerUser = game.users.get(String(form.ownerUserId.value || "").trim()) || null;
          const swornUser = game.users.get(String(form.swornToPlayerUserId.value || "").trim()) || null;
          const protectedUser = game.users.get(String(form.marriageProtectedPlayerUserId.value || "").trim()) || null;
          let buildings = [];
          let oldBuildingCount = currentBuildingCount;
          let developmentLevel = currentBuildingCount;
          let developmentLabel = currentDevelopment.label;
          let population = existing.population ?? "";
          if (!isSea) {
            for (const line of BUILDING_LINES) {
              const value = String(form[`line_${line.key}`]?.value || "").trim();
              if (value) buildings.push(value);
            }
            if (buildings.length > 4) {
              ui.notifications.warn(`A tile may only have 4 building lines. You selected ${buildings.length}.`);
              return false;
            }
            developmentLevel = Math.max(0, Math.min(4, buildings.length));
            developmentLabel = DEVELOPMENT_LEVELS[developmentLevel]?.label || DEVELOPMENT_LEVELS[0].label;
    ...
    default: "save"
  }, { width: 820, height: 920, resizable: true }).render(true);
}
...
function getHoldingsForUser(user) {
  ...
  function withoutArmyTroopResources(resourceMap = {}) {
    const resources = normalizeResourceMap(resourceMap);
    const troopKeys = new Set([...ARMY_TROOP_TYPES.map(troop => troop.key), "Siege"]);
    for (const key of Object.keys(resources)) if (troopKeys.has(key)) delete resources[key];
    return resources;
  }

  function trainingCapacityText(capacity = {}, emptyText = "Mob only") {
    const parts = [];
    for (const troop of ARMY_TROOP_TYPES) {
      if (troop.key === "Mob") continue;
      const value = Math.max(0, Number(capacity[troop.key] || 0));
  ...
  function holdingsSummary(entries) {
    ...
    let stockpile = {};
    let income = {};
    let rawManpowerCurrent = 0;
    let rawManpowerMax = 0;
    ...
    for (const entry of entries) {
      ...
      stockpile = addResourceMaps(stockpile, withoutArmyTroopResources(getHouseResourceStockpile(house)));
      income = addResourceMaps(income, withoutArmyTroopResources(getTileTotalIncome(house, getClock())));
    }
    for (const entry of landEntries) {
      const house = entry.house || getHouseData(entry.drawing) || {};
      rawManpowerCurrent += getProvinceManpowerCurrent(house);
      rawManpowerMax += getProvinceManpowerMax(house);
    }
    const influence = getInfluenceSummaryForEntries(landEntries);
    const manpowerMax = Math.floor(rawManpowerMax * influence.levyMultiplier);
    const manpowerCurrent = Math.min(rawManpowerCurrent, manpowerMax);
    const trainingCapacity = getTrainingCapacityForEntries(landEntries);
    const siegeCapacityTotal = getSiegeCapacityForEntries(landEntries);
    return { population, treasury, buildings, stockpile, income, influence, manpowerCurrent, manpowerMax, rawManpowerCurrent, rawManpowerMax, trainingCapacity, siegeCapacityTotal };
  }
  ...
  function getHouseInfluenceScoreFromEntries(entries = []) {
    let score = 10;
    for (const entry of entries) {
      const house = entry.house || getHouseData(entry.drawing) || {};
      const stockpile = getHouseResourceStockpile(house);
      const value = Number(stockpile.Influence || 0);
  ...
  function getInfluenceMood(score = 10) {
    ...
    if (value >= 7) return "Shaky";
    if (value >= 4) return "Unrest";
    if (value >= 1) return "Dangerous";
    return "Civil Revolt Risk";
  }

  function getInfluenceSummaryForEntries(entries = []) {
    const score = getHouseInfluenceScoreFromEntries(entries);
    const levyMultiplier = getInfluenceLevyMultiplier(score);
    const levyCapPercent = Math.round(levyMultiplier * 100);
    const levyPenaltyPercent = Math.max(0, 100 - levyCapPercent);
    return { score, mood: getInfluenceMood(score), levyMultiplier, levyCapPercent, levyPenaltyPercent };
  }
  ...
  function getCommittedSiegeCapacityForUser(user) {
    if (!user) return 0;
    let used = 0;
    for (const token of getArmyTokens()) {
      const piece = getWorldPiece(token) || {};
    ...
    for (const token of getCharacterTokens()) {
      const pending = getPendingArmyMuster(token);
      if (!pending || pending.status !== "pending") continue;
      const ownerId = String(pending.ownerUserId || pending.requesterUserId || "");
      const ownerName = String(pending.ownerUserName || pending.requesterUserName || "");
      if ((ownerId && ownerId === String(user.id)) || (!ownerId && ownerName && normalize(ownerName) === normalize(user.name))) used += getSiegeEquipmentCapacityUsed(getSiegeEquipment(pending));
    }
    return Math.max(0, Math.floor(used));
  }
  ...
  async function showHoldings() {
    ...
    const rows = entries.map(entry => {
      ...
      const incomeText = resourceMapToText(withoutArmyTroopResources(getTileTotalIncome(house, getClock())));
      const politicalHouse = getTileDisplayHouseLabelForTooltip(tile, house);
      const localHouse = String(house.house || tile.house || tile.owner || "None").trim();
      const allegiance = String(house.allegiance || tile.allegiance || "Independent").trim();
      const localLine = normalize(localHouse) && normalize(localHouse) !== normalize(politicalHouse) ? `<br><span style="opacity:0.75;">Local House: ${escapeHtml(localHouse)}</span>` : "";
      return `
        <tr>
          ...
          <td style="padding:5px 7px;border:1px solid #777;"><strong>${escapeHtml(politicalHouse)}</strong>${localLine}<br><span style="opacity:0.75;">Ruler: ${escapeHtml(house.lord || "None")}</span><br><span style="opacity:0.75;">Allegiance: ${escapeHtml(allegiance)}</span></td>
  ...
  function getRealmStockpileForEntries(entries = []) {
    let total = {};
    for (const entry of entries) {
      const house = getHouseData(entry.drawing) || entry.house || {};
      total = addResourceMaps(total, getHouseResourceStockpile(house));
    }
    return normalizeResourceMap(total);
  }

  function cleanResourceNumber(value) {
    const number = Number(value || 0);
    if (!Number.isFinite(number)) return 0;
    return Math.round(number * 1000) / 1000;
  }

  async function debitCostFromRealmStockpile(entries = [], costMap = {}, reason = "Realm resource cost") {
    const landEntries = entries.filter(entry => entry?.drawing?.document && !isSeaByTile(entry.tile));
    if (!landEntries.length) return { before: {}, after: {}, deltas: [], deficit: normalizeResourceMap(costMap) };

    const before = getRealmStockpileForEntries(landEntries);
    const normalizedCost = normalizeResourceMap(costMap);
    const deltasById = new Map(landEntries.map(entry => [entry.drawing.document.id, {}]));
    const deficit = {};

    for (const [resource, rawCost] of Object.entries(normalizedCost)) {
      const cost = Math.max(0, Number(rawCost || 0));
      if (!cost) continue;

      const rows = landEntries.map(entry => {
        const house = getHouseData(entry.drawing) || entry.house || {};
        const stockpile = getHouseResourceStockpile(house);
        return {
          entry,
          id: entry.drawing.document.id,
          balance: Math.max(0, Number(stockpile[resource] || 0))
        };
      });

      const available = rows.reduce((sum, row) => sum + row.balance, 0);
      let remaining = cost;

      if (available > 0) {
        const affordable = Math.min(cost, available);
        let distributed = 0;
        const positiveRows = rows.filter(row => row.balance > 0);

        positiveRows.forEach((row, index) => {
          let deduction;
          if (index === positiveRows.length - 1) {
            deduction = affordable - distributed;
          } else {
            deduction = affordable * (row.balance / available);
            deduction = Math.min(row.balance, cleanResourceNumber(deduction));
            distributed += deduction;
          }
          deduction = Math.max(0, Math.min(row.balance, cleanResourceNumber(deduction)));
          if (!deduction) return;
          const delta = deltasById.get(row.id);
          delta[resource] = cleanResourceNumber(Number(delta[resource] || 0) - deduction);
        });

        remaining = cleanResourceNumber(cost - affordable);
      }

      // Preserve the old behaviour that upkeep is always charged, even if the realm
      // cannot afford it. Instead of throwing the entire debt onto one arbitrary
      // province, spread the shortfall evenly across the realm.
      if (remaining > 0) {
        deficit[resource] = remaining;
        const share = remaining / landEntries.length;
        let assigned = 0;
        landEntries.forEach((entry, index) => {
          const amount = index === landEntries.length - 1 ? remaining - assigned : cleanResourceNumber(share);
          assigned += amount;
          const delta = deltasById.get(entry.drawing.document.id);
          delta[resource] = cleanResourceNumber(Number(delta[resource] || 0) - amount);
        });
      }
    }
    ...
    const appliedDeltas = [];
    for (const entry of landEntries) {
      const delta = normalizeResourceMap(deltasById.get(entry.drawing.document.id) || {});
      if (!hasAnyResources(delta)) continue;
      await applyResourceDeltaToTile(entry, delta, reason);
      appliedDeltas.push({ tileName: getTileName(entry), delta });
    }

    const after = {};
    for (const [key, value] of Object.entries(before)) after[key] = cleanResourceNumber(value);
    for (const [key, value] of Object.entries(normalizedCost)) after[key] = cleanResourceNumber(Number(after[key] || 0) - Number(value || 0));
  ...
  async function debitMilitaryUpkeepForRound(clock = getClock(), { scope = "all", userId = "", silent = false } = {}) {
    ...
    for (const bucket of ownerCosts.values()) {
      ...
      const realmDebit = await debitCostFromRealmStockpile(
        entries,
        bucket.cost,
  ...
  async function collectEconomyForRound(clock = getClock(), { scope = "all", userId = "", force = false, silent = false } = {}) {
    ...
    for (const entry of entries) {
      ...
      const incomeLine = `<li><strong>${escapeHtml(entry.tile.name || "Unnamed Tile")}</strong>: +${escapeHtml(resourceMapToText(income))}</li>`;
      totals = addResourceMaps(totals, income);
      applied++;
      lines.push(incomeLine);

      const playerUser = getPlayerUserForTileOwnership(entry.tile, house);
      const bucket = ensureEconomyBucket(playerBuckets, playerUser);
      if (bucket) {
        bucket.applied++;
        bucket.totals = addResourceMaps(bucket.totals, income);
        bucket.lines.push(incomeLine);
      }
    }

    const upkeepSummary = await debitMilitaryUpkeepForRound(clock, { scope, userId, silent: true });

    for (const detail of upkeepSummary.ownerDetails || []) {
      const user = getUserByIdOrName(detail.ownerId, detail.ownerName);
      const bucket = ensureEconomyBucket(playerBuckets, user);
      if (!bucket) continue;
      bucket.upkeepTotals = addResourceMaps(bucket.upkeepTotals, detail.cost || {});
      const forceText = (detail.forceLines || []).map(escapeHtml).join("; ");
      const realmBalanceText = detail.realmBefore && detail.realmAfter
        ? `<br><span style="opacity:0.85;">Realm: ${escapeHtml(goldFoodOnlyText(detail.realmBefore, "None"))} → ${escapeHtml(goldFoodOnlyText(detail.realmAfter, "None"))}</span>`
        : "";
      bucket.upkeepLines.push(`<li><strong>${escapeHtml(detail.ownerName || user.name)}</strong>: -${escapeHtml(resourceMapToText(detail.cost || {}, "None"))}${detail.paidFrom ? ` from ${escapeHtml(detail.paidFrom)}` : ""}${realmBalanceText}${forceText ? `<br><span style="opacity:0.85;">${forceText}</span>` : ""}</li>`);
    }
    ...
    if (!silent) {
      ...
      const fullContent = buildEconomyCollectionContent({
        title: "Economy Collected — GM Details",
        dateLabel,
        seasonMultiplier,
        applied,
        totals,
        lines,
        upkeepTotals: upkeepSummary.totals || {},
        upkeepLines: upkeepSummary.lines || []
      });

      if (gmIds.length) {
        await ChatMessage.create({ speaker: ChatMessage.getSpeaker({ alias: "Crown Economy" }), whisper: gmIds, content: fullContent });
      } else {
        await ChatMessage.create({ speaker: ChatMessage.getSpeaker({ alias: "Crown Economy" }), content: fullContent });
      }

      const shouldNotifyPlayers = scope === "all" || scope === "player";
      if (shouldNotifyPlayers) {
        for (const bucket of playerBuckets.values()) {
          const requestedPlayerOnly = scope === "player" && userId && String(bucket.user.id) !== String(userId);
          if (requestedPlayerOnly) continue;
          const hasIncome = bucket.lines.length > 0;
          const hasUpkeep = bucket.upkeepLines.length > 0;
          if (!hasIncome && !hasUpkeep) continue;
          const playerContent = buildEconomyCollectionContent({
            title: "Your Economy Collected",
            dateLabel,
            seasonMultiplier,
            applied: bucket.applied,
            totals: bucket.totals,
            lines: bucket.lines,
            upkeepTotals: bucket.upkeepTotals,
            upkeepLines: bucket.upkeepLines
          });
          await ChatMessage.create({ speaker: ChatMessage.getSpeaker({ alias: "Crown Economy" }), whisper: [bucket.user.id], content: playerContent });
        }
      }
  ...
  async function collectEconomy() {
    ...
    const result = await new Promise(resolve => {
      new Dialog({
        ...
        content: `<form>
          <p>Collect resource income into province balances for the current world round. For military upkeep, all controlled province balances are treated as one Realm Stockpile, so armies and navies are paid from the realm rather than an arbitrary province.</p>
          <div class="form-group">
            <label>Scope</label>
            <select name="scope" style="width:100%;">
  ...
  async function exportRealm() {
    ...
    for (const drawing of canvas.drawings.placeables) {
      ...
      rows.push({
        ...
        developmentGoldBonus: getDevelopmentEconomyBonus(house, getClock()).gold,
        developmentFoodBonus: getDevelopmentEconomyBonus(house, getClock()).food,
  ...
  function getCanonicalHouseNameForPlayer(userId = "", userName = "") {
    ...
    const addCandidate = value => {
      ...
      if (!name || isGenericNeutralLocalHouse(name)) return;
      const key = normalize(name);
      const row = names.get(key) || { name, count: 0 };
      row.count += 1;
      names.set(key, row);
    };

    // Character House identity is strongest evidence.
    for (const token of getCharacterTokens()) {
      const controller = getCharacterControllerIdentity(token);
      ...
      const character = getCharacterDataFromToken(token) || {};
      const piece = getWorldPiece(token) || {};
      addCandidate(character.house || piece.house || piece.faction || "");
    }

    // Fall back to the player's already-controlled holdings.
    for (const entry of getWorldTileEntries()) {
      if (isSeaByTile(entry.tile)) continue;
      const house = getHouseData(entry.drawing) || {};
      const tileOwnerId = String(getTileOwnerUserId(entry.tile, house) || "").trim();
      const tileOwnerName = normalize(getTileOwnerUserName(entry.tile, house) || "");
      const matches = wantedId
        ? tileOwnerId === wantedId
        : Boolean(wantedName && tileOwnerName === wantedName);
      if (!matches) continue;
      addCandidate(house.allegiance || entry.tile?.allegiance || "");
      addCandidate(house.house || entry.tile?.house || entry.tile?.owner || "");
    }

    const rows = [...names.values()].sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));
  ...
  async function repairPlayerProvinceAllegiances() {
    ...
    for (const entry of getWorldTileEntries()) {
      ...
      // A real player controller is the strongest evidence that this province is
      // player-controlled. Older partial transfers could leave ownershipType as
      // Neutral/Unaligned even though ownerUserId/playerOwnerUserId was correctly saved.
      if (!controllerId && !controllerName) continue;

      const tileAllegiance = String(worldTile.allegiance || "").trim();
      const houseAllegiance = String(house.allegiance || "").trim();
      const localHouse = String(house.house || worldTile.house || worldTile.localHouse || worldTile.owner || "").trim();
      const staleOwnershipType = ownershipType !== "player";
      const staleAllegiance = isGenericNeutralLocalHouse(tileAllegiance) || isGenericNeutralLocalHouse(houseAllegiance);
      const staleLocalHouse = isGenericNeutralLocalHouse(localHouse);
      if (!staleOwnershipType && !staleAllegiance && !staleLocalHouse) continue;

      const canonical = getCanonicalHouseNameForPlayer(controllerId, controllerName);
      if (!canonical) {
        unresolved++;
        unresolvedRows.push(getTileName(entry));
        continue;
      }

      const genericLocal = isGenericNeutralLocalHouse(localHouse);

      worldTile.ownershipType = "Player";
      worldTile.ownerType = "Player";
      worldTile.allegiance = canonical;
      worldTile.publicOwnerLabel = canonical;
      if (genericLocal) {
        worldTile.owner = canonical;
        worldTile.house = canonical;
        worldTile.localHouse = canonical;
      }

      house.ownershipType = "Player";
      house.ownerType = "Player";
      house.allegiance = canonical;
      house.publicOwnerLabel = canonical;
      if (genericLocal) house.house = canonical;

      const now = new Date().toISOString();
      const audit = {
        at: now,
        by: game.user.name,
        source: "Repair Player Allegiance",
        actingUserId: controllerId,
        actingUserName: controllerName,
        allegiance: canonical
      };
      worldTile.lastOwnershipChange = worldTile.lastOwnershipChange || audit;
      house.lastOwnershipChange = house.lastOwnershipChange || audit;
      worldTile.updatedAt = now;
      worldTile.updatedBy = game.user.name;
      house.updatedAt = now;
      house.updatedBy = game.user.name;
      ...
      repaired++;
      const repairedParts = [];
      if (staleOwnershipType) repairedParts.push("ownership");
      if (staleAllegiance) repairedParts.push("allegiance");
      if (staleLocalHouse) repairedParts.push("House");
      repairedRows.push(`${getTileName(entry)} → ${canonical}${repairedParts.length ? ` (${repairedParts.join(", ")})` : ""}`);
    }

    redrawPoliticalOverlay();
    const gmUsers = game.users.contents.filter(user => user.isGM).map(user => user.id);
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ alias: "Crown Province Repair" }),
      whisper: gmUsers.length ? gmUsers : undefined,
      content: `<h2>Player Province Identity Repair</h2>
        <p><strong>Repaired:</strong> ${escapeHtml(repaired)}</p>
        <p><strong>Unresolved:</strong> ${escapeHtml(unresolved)}</p>
        ${repairedRows.length ? `<p><strong>Updated:</strong><br>${repairedRows.map(row => escapeHtml(row)).join("<br>")}</p>` : ""}
        ${unresolvedRows.length ? `<p><strong>Could not infer a single House for:</strong><br>${unresolvedRows.map(row => escapeHtml(row)).join("<br>")}</p>` : ""}`
    });

    ui.notifications.info(`Player province identity repair complete: ${repaired} repaired, ${unresolved} unresolved.`);
    return { repaired, unresolved, repairedRows, unresolvedRows };
  }
  ...
  async function auditProvinceHouseData() {
    ...
    for (const entry of getWorldTileEntries()) {
      ...
      const houseNameGeneric = isGenericNeutralLocalHouse(houseName);
      const tileHouseGeneric = isGenericNeutralLocalHouse(tileHouse);
      if (houseName && tileHouse && !houseNameGeneric && !tileHouseGeneric && normalize(houseName) !== normalize(tileHouse)) {
        tileIssues.push(`Named local House mismatch: House Data '${houseName}' vs World Tile '${tileHouse}'`);
      }
      ...
      const resolvedOwnershipType = normalize(inferOwnershipType(worldTile, house));
      if (resolvedOwnershipType === "player" && !controllerId) tileIssues.push("marked Player-owned but has no controller user id");
      if (controllerId && resolvedOwnershipType !== "player") {
        tileIssues.push(`has player controller '${controllerName || controllerId}' but ownership type is '${resolvedOwnershipType || "blank"}'; expected Player`);
      }

      const politicalAllegiance = String(house.allegiance || worldTile.allegiance || "").trim();
      if (controllerId && isGenericNeutralLocalHouse(politicalAllegiance)) {
        const inferred = getCanonicalHouseNameForPlayer(controllerId, controllerName);
        tileIssues.push(inferred
          ? `player-controlled province still has generic political allegiance '${politicalAllegiance || "blank"}'; expected ${inferred}`
          : `player-controlled province still has generic political allegiance '${politicalAllegiance || "blank"}'; House could not be inferred automatically`);
      }

      const localIdentity = String(house.house || worldTile.house || worldTile.localHouse || worldTile.owner || "").trim();
      if (controllerId && isGenericNeutralLocalHouse(localIdentity)) {
        const inferred = getCanonicalHouseNameForPlayer(controllerId, controllerName);
        tileIssues.push(inferred
          ? `player-controlled province still has generic local House '${localIdentity || "blank"}'; expected ${inferred}`
          : `player-controlled province still has generic local House '${localIdentity || "blank"}'; House could not be inferred automatically`);
      }
      ...
      if (tileIssues.length) issues.push({
        tileName,
        houseName: houseName || tileHouse || "—",
        allegiance: String(house.allegiance || worldTile.allegiance || inferOwnershipType(worldTile, house) || "—").trim() || "—",
    ...
    const content = issues.length
      ? `<p>Checked <strong>${escapeHtml(checked)}</strong> land provinces and found <strong>${escapeHtml(issues.length)}</strong> with possible data problems.</p><div style="max-height:520px;overflow-y:auto;">${issues.map(item => `<div style="border:1px solid #777;border-radius:5px;padding:8px;margin:6px 0;"><strong>${escapeHtml(item.tileName)}</strong><br><strong>Local House:</strong> ${escapeHtml(item.houseName)} &nbsp; <strong>Allegiance:</strong> ${escapeHtml(item.allegiance)} &nbsp; <strong>Controller:</strong> ${escapeHtml(item.controllerName)}<br>${item.issues.map(issue => `• ${escapeHtml(issue)}`).join("<br>")}</div>`).join("")}</div>`
      : `<p>Checked <strong>${escapeHtml(checked)}</strong> land provinces. No House/controller/manpower inconsistencies were detected.</p>`;

    new Dialog({
      title: "Province / House Data Audit",
      content,
      buttons: {
        repair: {
          label: "Repair Player Province Identity",
          callback: async () => { await repairPlayerProvinceAllegiances(); }
        },
        close: { label: "Close" }
      },
      default: "close"
