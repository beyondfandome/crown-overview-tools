...
function getHoldingsForUser(user) {
  ...
  async function debitMilitaryUpkeepForRound(clock = getClock(), { scope = "all", userId = "", silent = false } = {}) {
    ...
    for (const bucket of ownerCosts.values()) {
      ...
      applied++;
      const lineHtml = `<li><strong>${escapeHtml(bucket.ownerName || bucket.ownerId || "Unknown")}</strong>: -${escapeHtml(resourceMapToText(bucket.cost))} from ${escapeHtml(getTileName(treasuryEntry))}<br><span style="opacity:0.85;">${bucket.forceLines.map(escapeHtml).join("; ")}</span></li>`;
      lines.push(lineHtml);
      ownerDetails.push({ ownerId: bucket.ownerId || "", ownerName: bucket.ownerName || "", cost: bucket.cost, forceLines: [...bucket.forceLines], paidFrom: getTileName(treasuryEntry), lineHtml });
    }
  ...
  async function collectEconomyForRound(clock = getClock(), { scope = "all", userId = "", force = false, silent = false } = {}) {
    ...
    for (const detail of upkeepSummary.ownerDetails || []) {
      ...
      bucket.upkeepLines.push(`<li><strong>${escapeHtml(detail.ownerName || user.name)}</strong>: -${escapeHtml(resourceMapToText(detail.cost || {}, "None"))}${detail.paidFrom ? ` from ${escapeHtml(detail.paidFrom)}` : ""}${forceText ? `<br><span style="opacity:0.85;">${forceText}</span>` : ""}</li>`);
    }
