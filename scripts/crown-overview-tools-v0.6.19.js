(() => {
  ...
  function getMineFoodBuildingConflict(house = {}, targetBuilding = "") {
    const state = getBuildingLineState(house);
    const minePresent = state.has("mine");
    const target = normalize(targetBuilding);
    const blockedFoodTier3 = new Set([normalize("Granary"), normalize("Slaughterhouses"), normalize("Deep Sea Fishing Boats")]);

    if (minePresent && blockedFoodTier3.has(target)) {
      return `${targetBuilding} cannot be built on a tile that has a Mine, Deep Mine, or Mining Complex.`;
    }

    const targetMeta = getBuildingMetaByName(targetBuilding);
    if (targetMeta?.line?.key === "mine") {
      for (const name of ["Granary", "Slaughterhouses", "Deep Sea Fishing Boats"]) {
        const meta = getBuildingMetaByName(name);
        const current = meta ? state.get(meta.line.key) : null;
        if (current && Number(current.level.level) >= 3) {
          return `${targetBuilding} cannot be built on a tile that already has ${current.level.name}.`;
        }
      }
    }
    return "";
