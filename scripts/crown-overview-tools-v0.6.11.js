...
function getPoliticalOverlayStyle(entry, playerColors, seenTileIds = null) {
  ...
  // Once seen, neutral / NPC territory uses a dedicated grey rather than a player colour.
  ...
  // A player keeps the same political colour everywhere on the map, including conquests
  // outside their starting region. This makes one realm visually coherent.
...
function getHoldingsForUser(user) {
  ...
  async function repairPlayerProvinceAllegiances() {
    ...
    for (const entry of getWorldTileEntries()) {
      ...
      const ownershipType = normalize(inferOwnershipType(worldTile, house));
      if (ownershipType !== "player" || (!controllerId && !controllerName)) continue;
      ...
      if (!staleAllegiance && !staleLocalHouse) continue;
      ...
      await replaceProvincePoliticalFlags(entry, worldTile, house);
      repaired++;
      repairedRows.push(`${getTileName(entry)} → ${canonical}`);
    }
  ...
  async function auditProvinceHouseData() {
    ...
    for (const entry of getWorldTileEntries()) {
      ...
      if (normalize(worldTile.ownershipType || house.ownershipType || "") === "player" && controllerId && isGenericNeutralLocalHouse(politicalAllegiance)) {
      ...
      if (normalize(worldTile.ownershipType || house.ownershipType || "") === "player" && controllerId && isGenericNeutralLocalHouse(localIdentity)) {
        const inferred = getCanonicalHouseNameForPlayer(controllerId, controllerName);
        tileIssues.push(inferred
