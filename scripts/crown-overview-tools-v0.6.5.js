...
function removePanel() {
  ...
  async function applyDiplomaticTakeover({ token, piece, entry, actingUserId, actingUserName }) {
    ...
    if (piece.embarked || piece.embarkedFleetTokenId || character.embarkedFleetTokenId) throw new Error("Diplomacy is unavailable while this character is embarked at sea.");
  ...
  async function diplomaticTakeover() {
    ...
    if (piece.embarked || piece.embarkedFleetTokenId || character.embarkedFleetTokenId) { ui.notifications.warn("Diplomacy is unavailable while this character is embarked at sea."); return; }
...
function getPoliticalOverlayOwnerShadeIndex(regionOwners, regionName, ownerKey) {
  const key = normalize(regionName || "unknown");
  const owners = regionOwners.get(key) || [];
  const idx = Math.max(0, owners.indexOf(ownerKey));
  const palette = [42, 56, 68, 30];
  return palette[idx % palette.length];
}

function getPoliticalOverlayStyle(entry, regionOwners, seenTileIds = null) {
  const house = getHouseData(entry.drawing) || {};
  const tile = entry.tile || {};
  ...
  if (ownershipType === "neutral" || ownershipType === "npc" || !ownerUser) {
    return {
      ...
      fillAlpha: isSeen ? 0.22 : 0.12,
      lineColor: hslToHex(0, 0, 70),
      lineAlpha: 0.55,
  ...
  const lightness = getPoliticalOverlayOwnerShadeIndex(regionOwners, regionName, ownerKey);
  return {
    fillColor: hslToHex(hue, 62, lightness),
    fillAlpha: 0.24,
    lineColor: hslToHex(hue, 68, Math.max(24, lightness - 16)),
    lineAlpha: 0.75,
    lineWidth: 2
  };
}
...
function redrawPoliticalOverlay() {
  ...
  for (const entry of entries) {
    ...
    const key = normalize(tile.region || house.region || "unknown");
    const ownerKey = normalize(ownerUser.id || ownerUser.name || "owner");
    if (!regionOwners.has(key)) regionOwners.set(key, []);
    if (!regionOwners.get(key).includes(ownerKey)) regionOwners.get(key).push(ownerKey);
  }
  for (const owners of regionOwners.values()) owners.sort();
  for (const entry of entries) {
    const style = getPoliticalOverlayStyle(entry, regionOwners, seenTileIds);
