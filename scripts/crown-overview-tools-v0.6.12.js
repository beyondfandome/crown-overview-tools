...
function removePanel() {
  ...
  async function summonArmy() {
    ...
    try { validateArmyMusterCapacity(token, muster); await reserveManpowerForMuster(token, muster); } catch (err) { ui.notifications.error(err.message || "Could not reserve manpower."); return; }
    await savePendingArmyMuster(token, muster);
    await ChatMessage.create({ speaker: ChatMessage.getSpeaker({ alias: "Crown Armies" }), content: `<h2>Army Muster Requested</h2><p><strong>Commander:</strong> ${escapeHtml(muster.linkedCharacterName)}</p><p><strong>Army:</strong> ${escapeHtml(muster.armyName)}</p><p><strong>Strength:</strong> ${escapeHtml(muster.totalStrength.toLocaleString())}</p><p><strong>Composition:</strong> ${escapeHtml(armyCompositionText(muster.composition))}</p><p><strong>Siege Equipment:</strong> ${escapeHtml(siegeEquipmentText(getSiegeEquipment(muster)))}</p><p><strong>Upkeep:</strong> ${escapeHtml(resourceMapToText(muster.upkeep))}</p><p><strong>Ready:</strong> ${escapeHtml(muster.readyDateLabel || "next round")}</p>` });
  }
  ...
  async function handleArmyMusterRequest(message) {
    ...
    try { validateArmyMusterCapacity(token, message.muster); await reserveManpowerForMuster(token, message.muster); } catch (err) { ui.notifications.error(err.message || "Could not reserve manpower for army muster."); return; }
